...
| opencode      | Full        | `XDG_DATA_HOME`     | `~/.opencode-<name>` (+ `~/.opencode`) | `XDG_DATA_HOME=... opencode auth login` | Account dir is the XDG data root; auth lands at `<dir>/opencode/auth.json`. Config stays global via `XDG_CONFIG_HOME`. XDG_DATA_HOME is honored on Windows too: opencode resolves paths via the npm `xdg-basedir` package (no platform branch, fallback `<home>/.local/share` on every OS - never `%APPDATA%`), verified 2026-07-14 against sst/opencode `packages/core/src/global.ts` |
...
4. **Resume is a proper batch turn, built by the shared spawn helper.** The respawn handler in `App.tsx` calls `buildAccountSwitchRespawnConfig()` (`src/renderer/utils/accountSwitchRespawn.ts`), which routes through the same `buildSpawnConfigForAgent()` helper as other spawn sites so the replayed turn is byte-for-byte equivalent to a user-initiated one: prompt (and any recorded images) in the spawn config, `agentSessionId` for `--resume`, the tab's permission mode, per-tab model/effort overrides, the Maestro system prompt via `prepareMaestroSystemPrompt()`, and the Windows stdin transport flags from `getStdinFlags()` (critical here: the respawn replays exactly the long interrupted prompts that would truncate on the ~8 KB cmd.exe argv). Prompts (plus images) are recorded in BOTH `process:write` (interactive/PTY paths) and `process:spawn` (batch paths, which is how desktop Claude turns actually deliver prompts), and the payload reaches the renderer as `lastPrompt`/`lastImages` on `account:switch-respawn`. Do NOT hand-roll a spawn config in the respawn path; extend the helper instead.
...
### 5.4 Windows support (implementation notes)

Everything below is the Windows-parity work done on `feat/account-multiplexing`. Each item names the code that owns the behavior; read that code before changing any of these paths.

1. **Account dirs use junctions plus copies, not symlinks** (`linkSharedResource()` in `account-setup.ts`). Creating a symlink on Windows requires `SeCreateSymbolicLinkPrivilege` (admin elevation or Developer Mode), which a stock install does not have, so `fs.symlink` fails with EPERM. Instead, shared DIRECTORIES get privilege-free NTFS directory junctions (`fs.symlink(..., 'junction')`) and shared FILES get plain copies (a file symlink would still need the privilege; junctions only work for directories). A junction failure is a hard error during creation (a half-provisioned dir is worse than no dir; the message points at network-share/FAT volumes, where junctions are unavailable). Consequences for validation/repair: `validateAccountSymlinks()` treats a present regular file as satisfying a resource (that is the NORMAL Windows state for copied files and must never be reported broken or missing), while junctions report `isSymbolicLink() === true` and take the normal symlink-check branch. Trade-off of copies: base-dir edits to a copied file (for example `settings.json`) do NOT propagate to account dirs on Windows the way symlinked files do on POSIX.

2. **Auth-recovery login spawns go through a shell on Windows** (`runProviderLogin()` in `account-auth-recovery.ts`). Provider CLIs on Windows are typically `.cmd`/`.ps1` npm shims that Node cannot exec directly, and a bare binary name is not PATH-resolved without a shell, so a plain `spawn(binary, args)` fails with ENOENT/EINVAL on every stock install. The fix mirrors `handle-spawn.ts`: resolve the preferred Windows shell via `getWindowsShellForAgentExecution({ customShellPath })` (honoring the user's configured shell, PowerShell over cmd.exe otherwise for its higher command-line length limit), quote with `escapeArgsForShell()`, and spawn with `shell:`. Two PowerShell-specific details: (a) the command line is prefixed with the `&` call operator when the shell is PowerShell (`isPowerShellShell()`), because PowerShell parses a leading quoted token - which is exactly what a binary path with spaces becomes after quoting - as a STRING LITERAL rather than a command, and errors on the trailing args; cmd.exe runs a quoted leading token as a command and gets no prefix. (b) On the login timeout the child is the shell wrapper, so a POSIX signal would orphan the login grandchild; Windows uses `taskkill /pid <pid> /t /f` to kill the whole tree. POSIX keeps the direct no-shell spawn and `child.kill('SIGTERM')`.

3. **Manual login commands render in PowerShell form on Windows** (`buildEnvLoginCommand()` in `src/shared/accountProviderMeta.ts`). The POSIX inline-env prefix (`CLAUDE_CONFIG_DIR="..." claude login`) is a syntax error in both cmd.exe and PowerShell. On Windows every copyable login command (Virtuosos panel, expired-account banner, auth-recovery failure toast) is built as `$env:VAR='C:\Users\...'; <binary> login`, with embedded single quotes doubled so the dir stays literal. These commands target PowerShell (Maestro's preferred Windows shell) and are NOT expected to work in cmd.exe.

4. **OpenCode multiplexing is SUPPORTED on Windows; the base data dir is resolved the xdg-basedir way** (`syncCredentialsFromBase()` in `account-setup.ts`; parity matrix in section 2.3). Phase 2 verified (2026-07-14, against sst/opencode `packages/core/src/global.ts` and sindresorhus/xdg-basedir) that opencode resolves its data root through the npm `xdg-basedir` package, which has NO platform branch: `$XDG_DATA_HOME` when set, else `<home>/.local/share` on EVERY OS, including Windows. It never uses `%APPDATA%`/`%LOCALAPPDATA%`. So injecting `XDG_DATA_HOME` isolates opencode accounts on Windows exactly as on POSIX, and credential sync reads the base auth file from `$XDG_DATA_HOME || <home>/.local/share` + `opencode/auth.json`. Any future change here must mirror xdg-basedir exactly; do not "fix" it to AppData.

5. **Path prefix checks are case-insensitive on Windows.** NTFS is case-insensitive, so matching `.claude-*`-style managed prefixes by exact case can both miss real dirs and defeat safety checks. `removeAccountDirectory()` (`account-setup.ts`) lowercases both sides of its managed-prefix safety check on Windows (bare base dirs like `~/.claude` never match a prefix and stay protected), and the CLI's filesystem discovery (`src/cli/services/account-reader.ts`) matches dir names case-insensitively on Windows. The CLI also probes Windows store locations (`%APPDATA%\Maestro\maestro-accounts.json`, plus the lowercase variant) before falling back to filesystem discovery.

6. **The switch respawn carries the Windows stdin transport flags** (design decision 4 above; `buildAccountSwitchRespawnConfig()` in `src/renderer/utils/accountSwitchRespawn.ts`). Normal turns deliver prompts via stdin on Windows because cmd.exe caps the command line around 8191 chars; the respawn path replays exactly the kind of long, multiline interrupted prompt that hits the cap, so it now computes `sendPromptViaStdin`/`sendPromptViaStdinRaw` through the shared helper instead of putting the prompt on argv. The respawn also pins the account dir with the PROVIDER's own env var (derived from `getAccountProviderMeta(session.toolType).envVar`: `CODEX_HOME`/`XDG_DATA_HOME`/`CLAUDE_CONFIG_DIR`), not a hardcoded `CLAUDE_CONFIG_DIR`, so a Codex/OpenCode switch no longer stamps a stray Claude var (the authoritative isolation still comes from main's `injectAccountEnv` keyed off `accountId`; this is belt-and-suspenders parity).

7. **CI test assertions build paths with `path.join`.** The two Windows CI legs originally failed only because account tests hardcoded POSIX `/` separators in expected paths; the product code was already separator-correct. Test expectations for paths must be built with `path.join` (or the helpers in `src/__tests__/helpers/pathExpect.ts`), never string literals with `/`.

8. **Remote (SSH) account validation is provider-aware** (`validateRemoteAccountDir()` in `account-setup.ts`). SSH remotes are POSIX, so the check keeps `test`/plain-symlink semantics, but it now derives the auth files and the resume-critical shared resource from `getAccountProviderMeta(inferProviderFromDir(configDir))` instead of hardcoding `.claude.json`/`projects`. A Codex remote probes `auth.json`/`config.toml` and checks the `config.toml` link; an OpenCode remote shares nothing, so `symlinksValid` is reported true without running a symlink probe. This runs on the remote host and never on Windows, so it is not a stock-Windows-install concern, but it is required for non-Claude accounts on SSH.

### 5.5 Needs manual Windows validation

The Windows-specific paths below are implemented against documented Windows behavior and locked in with `process.platform = 'win32'` unit tests, but they were written without a Windows machine. A Windows tester should confirm each item on a stock install (NO admin elevation, NO Developer Mode):

- [ ] **Account dir creation uses junctions/copies, and agents spawn against the new dir.** Virtuosos panel -> Create for claude-code: creation succeeds without elevation; in the new `%USERPROFILE%\.claude-<name>` dir, `dir` (or `fsutil reparsepoint query <dir>\projects`) shows `commands`, `ide`, `plans`, `plugins`, `todos`, `session-env`, `projects` as `<JUNCTION>` entries and `settings.json` / `CLAUDE.md` as regular copied files. Repeat for codex (`config.toml` copied, `prompts` junctioned). Then assign the account to a claude-code agent, send a turn, and confirm the child process actually sees the account dir: Process Monitor detail -> Virtuoso field, Task Manager -> details -> command line, or Process Explorer/Process Monitor environment inspection showing `CLAUDE_CONFIG_DIR=%USERPROFILE%\.claude-<name>`.
- [ ] **Symlink validate/repair handles junctions and copies.** In the Virtuosos panel run Validate on the freshly created dir (must report intact - copied files must NOT show as missing/broken). Delete the `commands` junction and `settings.json`, run Repair: the junction is recreated and the file re-copied, again without elevation.
- [ ] **Automatic auth recovery spawns the login CLI and resumes the turn.** With an account whose credentials were invalidated (corrupt `.credentials.json`), trigger a turn: the auth-recovery toast sequence fires and a `claude login` browser window actually opens (previously the spawn silently failed because the `.cmd` shim needs a shell). After authorizing, the interrupted prompt must be re-sent exactly once (`account:switch-respawn` with `reason: 'auth-recovery'`) and the agent resumes the conversation with `--resume`. Verify for codex (`codex login`) and opencode (`opencode auth login`) accounts too.
- [ ] **Manual login command is PowerShell syntax and authenticates.** The copyable command in the Virtuosos panel, the expired-account banner, and the auth-recovery failure toast must read `$env:CLAUDE_CONFIG_DIR='C:\Users\...'; claude login` (and the CODEX_HOME / XDG_DATA_HOME equivalents). Copy it from the panel, paste it into a PowerShell window, complete the browser OAuth, and confirm the credential file appears in the account dir (`.credentials.json` for claude, `auth.json` for codex, `opencode\auth.json` for opencode). It is NOT expected to work in cmd.exe.
- [ ] **Codex account isolation (`CODEX_HOME`) works end to end.** Create a codex account from the panel, assign it to a codex agent, send a turn, and confirm the child process env carries `CODEX_HOME=%USERPROFILE%\.codex-<name>` (Process Monitor detail or Process Explorer env inspection) and that `auth.json` written by login lands in the account dir, not `%USERPROFILE%\.codex`.
- [ ] **OpenCode credential sync finds the base auth file.** After a normal `opencode auth login` (no env overrides), confirm `%USERPROFILE%\.local\share\opencode\auth.json` exists (opencode uses xdg-basedir paths on Windows, not `%APPDATA%`), then use "Sync credentials" for an opencode account and verify `<account dir>\opencode\auth.json` appears.
- [ ] **OpenCode account isolation works end to end.** Spawn an opencode agent bound to a `~/.opencode-<name>` account and confirm (Process Monitor detail, or Process Explorer env inspection) the child sees `XDG_DATA_HOME=<account dir>` and writes session data under `<account dir>\opencode\` instead of `%USERPROFILE%\.local\share\opencode\`.
