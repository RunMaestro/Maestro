# Parallel E2E Coverage Campaign Plan
  ...
  Continue the Maestro E2E campaign from codex/full-e2e-coverage-campaign without running Playwright E2E
  tests. Current state is 638 declared E2E tests against a 3,025 active-test target, so the campaign needs
  about 2,387 additional active scenarios plus documented skipped/env-gated cases.
  ...
  - Create docs/e2e-parallel-campaign/ with:
      ...
      - coverage-ledger.md: matrix copied from docs/e2e-coverage-campaign.md, recalculated after each merge.
  ...
  - Keep docs/e2e-coverage-campaign.md as the canonical coverage matrix and update it after every accepted
    agent branch.

  - Keep docs/test-coverage-audit.md for campaign checkpoints; record that E2E tests are authored but
  ...
  --dangerously-bypass-approvals-and-sandbox. Every agent must update its docs/e2e-parallel-campaign/agents/
  ...
  - Orchestrator owns docs/e2e-parallel-campaign/orchestrator-status.md, coverage-ledger.md, broadcasts.md,
    and final merges.
  ...
  bypass-approvals-and-sandbox - < prompts/e2e-agents/<lane>.md" --name "maestro-e2e-<lane>" --no-autorestart
  --time
