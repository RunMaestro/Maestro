...
Work in `/Users/jeffscottward/Github/tools/Maestro-worktrees/e2e-autorun-ai-terminal` on branch `codex/e2e-autorun-ai-terminal`.

Objective: author E2E test coverage for Auto Run and Codex-only AI terminal workflows. Target about 273 additional matrix-backed active scenarios plus skipped/env-gated cases where live provider behavior is unavailable.
...
- Live provider E2E remains Codex-only.
- Update `docs/testing/e2e/parallel-campaign/agents/autorun-ai-terminal.md`.
- If you edit shared helpers, update `docs/testing/e2e/parallel-campaign/broadcasts.md`.
- Commit frequently. Prefer `gac -m "test(e2e-autorun-ai-terminal): ..."` if it stages only your lane work; otherwise use targeted `git add` and `git commit`.
