...
Work in `/Users/jeffscottward/Github/tools/Maestro-worktrees/e2e-wizard-settings-prompts` on branch `codex/e2e-wizard-settings-prompts`.

Objective: author E2E test coverage for New Agent Wizard, inline wizard, Settings, Director Notes, and prompt composer workflows. Target about 372 additional matrix-backed active scenarios plus skipped/env-gated cases for unavailable provider/account state.
...
- Update `docs/testing/e2e/parallel-campaign/agents/wizard-settings-prompts.md`.
- If you edit shared helpers, update `docs/testing/e2e/parallel-campaign/broadcasts.md`.
- Commit frequently. Prefer `gac -m "test(e2e-wizard-settings-prompts): ..."` if it stages only your lane work; otherwise use targeted `git add` and `git commit`.
