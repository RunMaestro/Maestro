...
- The current broad unit suite now passes with 1,246 files, 33,311 tests, and 0 skipped in the latest coverage run after removing stale skip blocks, removing one optional native-SQLite skip path, replaying compatible restored unit files from the green backup, and adapting current-upstream unit suites.
...
- The current top remaining coverage gaps are `renderer/App.tsx`, `TerminalOutput.tsx`, `messageHandlers.ts`, `FilePreview.tsx`, `main/index.ts`, `useInputProcessing.ts`, `symphony.ts`, `group-chat-router.ts`, `web/mobile/App.tsx`, and `autorun.ts`.
...
- Next work:
  - restore 100% unit coverage with current-upstream-compatible tests, continuing through the top uncovered files (`renderer/App.tsx`, `TerminalOutput.tsx`, `messageHandlers.ts`, `FilePreview.tsx`, `main/index.ts`, `useInputProcessing.ts`, `symphony.ts`, `group-chat-router.ts`, `web/mobile/App.tsx`, and `autorun.ts`),
