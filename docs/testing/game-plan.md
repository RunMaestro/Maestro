...
Last updated: 2026-06-18 18:48 EDT.
...
- The current broad unit suite now passes with 1,244 files, 33,201 tests, and 0 skipped in the latest coverage run after removing stale skip blocks, removing one optional native-SQLite skip path, replaying compatible restored unit files from the green backup, and adapting current-upstream unit suites.
...
- Full integration restoration is not complete yet: current local counts show 12 tracked integration files, 33 working-tree integration files, and 541 integration files preserved on `backup/full-e2e-coverage-campaign-119a0997b`.
...
- `AchievementsPanel.tsx`, `CuePanel.tsx`, `UsageDashboardPanel.tsx`, `GroupChatPanel.tsx`, `GitDiffViewer.tsx`, `AgentCreationSheet.tsx`, `GroupChatSetupSheet.tsx`, `NotificationSettingsSheet.tsx`, `AutoRunInline.tsx`, and `web/mobile/App.tsx` are improved after the isolated mobile panel/sheet/inline/App callback tranches; the current top remaining coverage gaps are `renderer/App.tsx`, `TerminalOutput.tsx`, `agents.ts`, `messageHandlers.ts`, `FilePreview.tsx`, `main/index.ts`, `useInputProcessing.ts`, `symphony.ts`, `group-chat-router.ts`, and `web/mobile/App.tsx`.
...
- Next work:
  - restore 100% unit coverage with current-upstream-compatible tests, continuing through the top uncovered files (`renderer/App.tsx`, `TerminalOutput.tsx`, `agents.ts`, `messageHandlers.ts`, `FilePreview.tsx`, `main/index.ts`, `useInputProcessing.ts`, `symphony.ts`, `group-chat-router.ts`, and `web/mobile/App.tsx`),
