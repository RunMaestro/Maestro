...
Last updated: 2026-06-19 00:00 EDT.
...
- The current broad unit suite now passes with 1,246 files, 33,463 tests, and 0 skipped reported in the latest coverage run after removing stale skip blocks, removing one optional native-SQLite skip path, replaying compatible restored unit files from the green backup, and adapting current-upstream unit suites.
...
- The current top remaining coverage gaps after the fresh broad coverage run are `main/index.ts`, `symphony.ts`, `group-chat-router.ts`, `web/mobile/App.tsx`, `LeaderboardRegistrationModal.tsx`, `MainPanel.tsx`, `useMainKeyboardHandler.ts`, `FeedbackChatView.tsx`, `web-server-factory.ts`, `TerminalOutput.tsx`, `useAppRemoteEventListeners.ts`, and `claude-session-storage.ts`.
