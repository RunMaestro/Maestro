...
Last updated: 2026-06-18 23:34 EDT.
...
- The current broad unit suite now passes with 1,246 files, 33,408 tests, and 0 skipped in the latest coverage run after removing stale skip blocks, removing one optional native-SQLite skip path, replaying compatible restored unit files from the green backup, and adapting current-upstream unit suites.
...
- Full integration restoration is not complete yet: current local counts under the broad integration-file matcher show 41 tracked files, 53 working-tree files, and 544 files preserved on `backup/full-e2e-coverage-campaign-119a0997b`.
...
- The current top remaining coverage gaps after the fresh broad coverage run are `main/index.ts`, `symphony.ts`, `group-chat-router.ts`, `web/mobile/App.tsx`, `autorun.ts`, `LeaderboardRegistrationModal.tsx`, `MainPanel.tsx`, `useMainKeyboardHandler.ts`, `FeedbackChatView.tsx`, `web-server-factory.ts`, `TerminalOutput.tsx`, and `useAppRemoteEventListeners.ts`.
...
- Next work:
  - restore 100% unit coverage with current-upstream-compatible tests, continuing through the top uncovered files (`FilePreview.tsx`, `group-chat-router.ts`, `symphony.ts`, `web/mobile/App.tsx`, `main/index.ts`, `useInputProcessing.ts`, `web-server-factory.ts`, `useMainKeyboardHandler.ts`, `autorun.ts`, `useAppRemoteEventListeners.ts`, and `TerminalOutput.tsx`),
