...
Maestro includes a CLI tool (`maestro-cli`) for sending messages to agents, browsing sessions, running playbooks, managing settings, and controlling resources from the command line, cron jobs, or CI/CD pipelines. Use Bun to invoke the bundled JavaScript CLI.
...
printf '#!/usr/bin/env bun\nbun "/Applications/Maestro.app/Contents/Resources/maestro-cli.js" "$@"\n' | sudo tee /usr/local/bin/maestro-cli && sudo chmod +x /usr/local/bin/maestro-cli

# Linux (deb/rpm installs to /opt)
printf '#!/usr/bin/env bun\nbun "/opt/Maestro/resources/maestro-cli.js" "$@"\n' | sudo tee /usr/local/bin/maestro-cli && sudo chmod +x /usr/local/bin/maestro-cli

# Windows (PowerShell as Administrator) - create a batch file
@"
@echo off
bun "%ProgramFiles%\Maestro\resources\maestro-cli.js" %*
"@ | Out-File -FilePath "$env:ProgramFiles\Maestro\maestro-cli.cmd" -Encoding ASCII
```

Alternatively, run it directly with Bun:

```bash
bun "/Applications/Maestro.app/Contents/Resources/maestro-cli.js" list groups
```
...
Each agent can have its own configuration for custom paths, CLI arguments, environment variables, and model overrides. See [Provider Notes](./provider-notes#custom-configuration) for provider-specific behavior.
...
- At least one supported AI agent CLI must be installed and in `PATH`; see [Provider Notes](./provider-notes) for documented provider capabilities.
