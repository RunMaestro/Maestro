...
4. **Claim** the oldest `ready` cards up to the work-in-progress cap, mark each `running`, open a run record, and spawn its assignee. The board is persisted *before* any spawn resolves, so a crash or the next tick sees the cards as `running` and never double-dispatches them.
