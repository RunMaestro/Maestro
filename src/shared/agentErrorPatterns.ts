 ...
 * used to carry six hand-written auth regexes that had already fallen behind the
 * eleven here, so it told users to run a login command for a provider that was
 * never claude-code.
 *
...
const CLAUDE_ERROR_PATTERNS: AgentErrorPatterns = {
	...
	// somewhere that cannot help, and `claude login` is not even a real command
	// (it is `claude auth login`). Maestro now signs the user in from the Auth
	// Recovery Modal, so no message here should name a terminal command.
	auth_expired: [
		{
