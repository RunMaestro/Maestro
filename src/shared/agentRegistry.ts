 ...
 *
 * This shared, data-only module is the authority for built-in agent IDs and
 * capabilities. Main-process CLI definitions retain only executable-specific
 * configuration, so consumers in every runtime can resolve the same IDs
 * without a runtime dependency on `src/main`.
 */
...
/**
 * Capability definitions for each supported built-in agent.
 *
 * Agents marked as PLACEHOLDER have not been integrated yet; their
 * capabilities remain conservative until verified against the CLI.
 */
...
/** Built-in IDs derived from the authoritative registry. */
export const AGENT_IDS = Object.freeze(Object.keys(AGENT_REGISTRY)) as readonly AgentId[];

/**
 * Legacy spellings accepted at the built-in resolution boundary.
 *
 * Callers must persist and expose the returned canonical ID, never an alias.
 */
...
/** Capability lookup retained for consumers that need the complete built-in map. */
export const AGENT_CAPABILITIES: Readonly<Record<AgentId, AgentCapabilities>> = AGENT_REGISTRY;

export function resolveAgentId(agentId: string): AgentId | undefined {
	const normalized = agentId.trim().toLowerCase();
	if (Object.prototype.hasOwnProperty.call(AGENT_REGISTRY, normalized)) {
		return normalized as AgentId;
	}
	return AGENT_ID_ALIASES[normalized as keyof typeof AGENT_ID_ALIASES];
}

export function isValidAgentId(agentId: string): agentId is AgentId {
	return Object.prototype.hasOwnProperty.call(AGENT_REGISTRY, agentId);
}

/**
 * Get capabilities for a built-in agent or conservative defaults for unknown/custom agents.
 */
export function getAgentCapabilities(agentId: string): AgentCapabilities {
	const resolved = resolveAgentId(agentId);
	return resolved ? AGENT_REGISTRY[resolved] : { ...DEFAULT_CAPABILITIES };
}
