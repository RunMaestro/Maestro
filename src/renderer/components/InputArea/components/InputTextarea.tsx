...
export const InputTextarea = memo(function InputTextarea({
	...
	// Style for a single mention chip in the LIVE overlay. When a recognized
	// mention is present, the overlay owns the visible glyphs and the native
	// textarea stays transparent underneath; during selection the overlay hides and
	// the textarea becomes the visible fallback. The decoration must add ZERO
	// inline advance or it drifts off the native caret (measured >200px on a long
	// path). Two tricks keep it width-exact:
	...
	return (
		<div className="relative flex items-start">
			...
			{overlayRendered && (
				<div
					...
					style={{
						...
						// The overlay paints the highlighted text while active. On selection
						// it hides and the native textarea becomes visible again.
			...
			<textarea
				...
				style={{
					...SHARED_TYPOGRAPHY,
					// Native text is the fallback. When the overlay is actively showing
					// mention highlights, it owns the visible glyphs so the text and chip
					// stay in the same visual layer.
					color: overlayVisible ? 'transparent' : theme.colors.textMain,
