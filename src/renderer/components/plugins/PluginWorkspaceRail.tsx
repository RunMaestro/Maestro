...
export function PluginWorkspaceRail({
	...
	if (projection.phase === 'loading') {
		return <div role="status" className="p-3 text-sm" style={{ color: theme.colors.textDim }}>Loading workspace sessions…</div>;
	}
	if (projection.phase === 'error' || projection.snapshot?.connection === 'error') {
		return (
			<div role="alert" className="p-3 text-sm" style={{ color: theme.colors.error }}>
				{projection.snapshot?.error ?? projection.error ?? 'Plugin workspace transport failed.'}
			</div>
		);
	}
	if (projection.snapshot?.connection === 'offline') {
		return <div role="status" className="p-3 text-sm" style={{ color: theme.colors.textDim }}>Offline — workspace transport is unavailable.</div>;
	}
	...
	return (
		<nav className="min-h-0 overflow-auto border-r p-2" aria-label="External sessions" style={{ borderColor: theme.colors.border }}>
			{sessions.length === 0 ? (
				<p className="p-2 text-sm" style={{ color: theme.colors.textDim }}>No external sessions.</p>
			) : (
				sessions.map((session) => {
					const reveal = (): void => {
						void source.reveal({ ownerPluginId, workspaceLocalId, snapshotToken: session.snapshotToken });
					};
					...
					return (
						...
						>
							...
							<span className="mt-1 flex items-center gap-2 text-xs" style={{ color: theme.colors.textDim }}>
								<span>{statusLabel(session.status)}</span>
								{session.unread > 0 && <span aria-label={`${session.unread} unread`}>{session.unread > 99 ? '99+' : session.unread}</span>}
								{session.pendingApproval && <span aria-hidden>!</span>}
							</span>
						</button>
					);
				})
