...
function AgentApprovalsInner({ theme, approvals, onRespond, inline = false }: AgentApprovalsProps) {
	...
	useEffect(() => {
		if (!inline || !request) return;
		inlineCardRef.current?.querySelector<HTMLElement>('input, textarea, button')?.focus();
	}, [inline, request?.id]);
	...
	const card = (
		<div
			ref={inline ? inlineCardRef : undefined}
			...
			className={
				inline
					? 'mx-4 mb-2 rounded-xl border p-4 shadow-lg'
					: 'w-[520px] max-w-[90vw] rounded-xl shadow-2xl border p-5'
			}
			style={{
				backgroundColor: theme.colors.bgSidebar,
				borderColor: inline ? `${theme.colors.warning}90` : theme.colors.border,
