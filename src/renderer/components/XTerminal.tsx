...
export const XTerminal = forwardRef<XTerminalHandle, XTerminalProps>(function XTerminal(
	...
	// Re-measure the cell once the web fonts have actually arrived.
	//
	// The terminal fonts (JetBrains Mono and friends) are pulled from Google
	// Fonts with `display=swap`, so the browser paints with a fallback and swaps
	// the real face in whenever the network delivers it. xterm measures its cell
	// size ONCE, inside `term.open()`. When the swap lands after that, every
	// glyph is drawn at its own advance inside a cell measured for a different
	// font - which is why login output rendered as `C l a u d e   C o d e`, with
	// the letters correct but the spacing stretched.
	//
	// Terminal tabs mostly dodged it by being opened long after the fonts had
	// settled, and by re-fitting on every show/hide. Nothing re-measured for a
	// terminal that mounts once inside a modal, so the stale pitch stuck.
	useEffect(() => {
		...
		// `fonts.ready` only waits for loads already in flight, so ask for this
		// terminal's own face explicitly first - otherwise a font nothing has
		// painted yet is still unrequested when `ready` resolves, and the cell is
		// measured against the fallback all over again. Bold is requested too: it
		// has its own face, and the login TUI leans on it heavily.
		const primaryFamily = fontFamily.split(',')[0]?.trim();
		const requested = primaryFamily
			? Promise.allSettled([
					fonts.load(`${fontSize}px ${primaryFamily}`),
					fonts.load(`bold ${fontSize}px ${primaryFamily}`),
				])
			: Promise.resolve();

		void Promise.resolve(requested)
			.catch(() => {
				// A family the platform cannot parse or find is not fatal - fall
				// through to `ready` and re-measure against whatever is in use.
			})
			.then(() => fonts.ready)
			.then(() => {
				if (cancelled) return;
				...
				// Force the re-measure with a real change rather than re-assigning
				// the same value, which xterm is free to treat as a no-op. Bumping
				// the size and putting it straight back leaves options as they were.
				const currentSize = term.options.fontSize ?? fontSize;
				term.options.fontSize = currentSize + 1;
				term.options.fontSize = currentSize;

				// The WebGL glyph cache was filled using the stale metrics, so it
				// has to go too or the old spacing survives the re-measure.
