...
export function registerProcessHandlers(deps: ProcessHandlerDependencies): void {
			...
			async (config: {
				...
				if (config.sessionSshRemoteConfig?.enabled) {
					...
					if (sshResult.config) {
						...
						// Single-quote the path to prevent shell command substitution ($(…)) on
						// the remote host. Embedded single-quotes are escaped via '"'"' so the
						// quoting is correct even for paths that contain a literal apostrophe.
						const workingDirOverride = config.sessionSshRemoteConfig.workingDirOverride;
						if (workingDirOverride) {
							sshArgs.unshift('-t');
							const safeDir = workingDirOverride.replace(/'/g, `'"'"'`);
							sshArgs.push(`cd '${safeDir}' && exec $SHELL`);
						}
	...
	let runCommandDeprecationWarned = false;
	ipcMain.handle(
		'process:runCommand',
		withIpcErrorLogging(
			...
			async (config: {
				...
				if (!runCommandDeprecationWarned) {
					runCommandDeprecationWarned = true;
					logger.warn(
