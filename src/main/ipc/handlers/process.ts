	...
	ipcMain.handle(
		...
		withIpcErrorLogging(
			...
			async (config: {
				...
				// Claude Code's `maestro-p` interactive wrapper is opt-in only: users who
				// want it set their Claude Code custom path to point at `maestro-p` directly.
				// Maestro never auto-routes claude spawns through it.
