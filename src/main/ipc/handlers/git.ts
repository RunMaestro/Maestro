	...
	ipcMain.handle(
		...
		createIpcHandler(
			...
			async (worktreePath: string, sshRemoteId?: string) => {
				...
				// Run independent git queries in parallel for better performance
				const [gitDirResult, gitCommonDirResult, branchResult, repoRootResult] =
					await Promise.all([
		...
		withIpcErrorLogging(
			...
			async (
				...
				if (pathExists) {
					...
					// Run both git queries in parallel since they operate on different directories
