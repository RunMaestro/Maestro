...
| Max output size      | No cap on capture itself. **CLI mode** truncates the forwarded message at **30 000 chars on Windows / 100 000 on POSIX** to stay under argv ceilings. **History** entries truncate `stdout` at 10 000 chars. Downstream `{{CUE_SOURCE_OUTPUT}}` sees the full captured stdout. |
...
| ----------------------------- | --------------------------------------------------------------------- |
| `{{CUE_SOURCE_OUTPUT}}`       | The command's captured stdout (full).                                 |
| `{{CUE_SOURCE_STATUS}}`       | `completed` \| `failed` \| `timeout`                                  |
| `{{CUE_SOURCE_EXIT_CODE}}`    | Numeric exit code (or `null` on spawn failure).                       |
...
Without `source_sub`, the chain also fires on every other agent.completed in that session.
