...
\`\`\``);
		...
		it('should count plus-style unchecked tasks', () => {
			vi.mocked(fs.readFileSync).mockReturnValue('+ [ ] Plus task\n+ [✓] Completed task');
...
- [ ] Real task`);
		...
		it('should extract plus-style unchecked tasks', () => {
			vi.mocked(fs.readFileSync).mockReturnValue('+ [ ] Plus task');

			expect(readDocAndGetTasks('/playbooks', 'tasks').tasks).toEqual(['Plus task']);
		});
