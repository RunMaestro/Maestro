...
describe('groupChatAutoRunRegistry', () => {
	...
	it('registers and consumes group chat autorun context once', () => {
		...
		expect(getAutoRunSessionForGroupChatParticipant('group-1', 'Planner')).toBe('session-1');
		...
		expect(getAutoRunSessionForGroupChatParticipant('group-1', 'Planner')).toBeUndefined();
	});

	it('preserves scoped completion metadata until consumed by the batch complete event', () => {
		registerGroupChatAutoRun('session-complete-a', 'group-a', 'Runner');
		registerGroupChatAutoRun('session-complete-b', 'group-b', 'Runner');

		expect(consumeGroupChatAutoRunForCompletion('session-complete-a')).toEqual({
			groupChatId: 'group-a',
			participantName: 'Runner',
		});
		expect(consumeGroupChatAutoRunForCompletion('session-complete-b')).toEqual({
			groupChatId: 'group-b',
			participantName: 'Runner',
		});

		expect(getAutoRunSessionForGroupChatParticipant('group-a', 'Runner')).toBeUndefined();
		expect(consumeCompletedGroupChatAutoRun('group-b', 'Runner')).toBe('session-complete-b');
		expect(consumeCompletedGroupChatAutoRun('group-b', 'Runner')).toBeUndefined();
		expect(consumeCompletedGroupChatAutoRun('group-a', 'Runner')).toBe('session-complete-a');
	});
