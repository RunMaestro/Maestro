...
describe('UnifiedHistoryTab integration', () => {
	...
	it('loads unified history, filters/searches, resumes entries, and updates detail state', async () => {
		...
		render(<UnifiedHistoryTab theme={theme} onResumeSession={onResumeSession} />);
	...
	it('loads additional pages on near-bottom scroll and refreshes for lookback changes', async () => {
		...
		render(<UnifiedHistoryTab theme={theme} />);
	...
	it('maps all-time and oversized default lookbacks to all-time requests', async () => {
		useSettingsStore.setState({
			directorNotesSettings: {
				provider: 'claude-code',
		...
		const { unmount } = render(<UnifiedHistoryTab theme={theme} />);
		...
		useSettingsStore.setState({
			directorNotesSettings: {
				provider: 'claude-code',
				defaultLookbackDays: 10_000,
			},
		});
		getHistoryMock().mockClear();
		getHistoryMock().mockResolvedValueOnce(
			response([entry({ summary: 'Oversized lookback entry' })])
		);
		render(<UnifiedHistoryTab theme={theme} />);
		...
		expect(screen.getByText('Oversized lookback entry')).toBeInTheDocument();
	});
	...
	it('exposes focus and escape handlers and handles load failures', async () => {
		...
		render(<UnifiedHistoryTab ref={ref} theme={theme} />);

		await waitFor(() =>
			expect(errorSpy).toHaveBeenCalledWith('Failed to load unified history:', expect.any(Error))
		);
		expect(
