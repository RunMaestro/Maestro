...
describe('speckit manager integration', () => {
	...
	it('refreshes upstream prompts through mocked GitHub, download, and unzip boundaries', async () => {
		vi.stubGlobal(
			'fetch',
					...
					assets: [
						{
							...
							browser_download_url: 'https://example.test/speckit.zip',
						},
					],
				}),
			})
		);
		mockHttpsDownload(200, 'https://cdn.example.test/speckit.zip');
		mocks.execAsync.mockImplementation(async (command: string) => {
			if (command.includes('unzip -l')) {
				return {
					stdout: [
						'Archive: speckit.zip',
						'  123  01-01-2026 00:00   spec-kit/.claude/commands/constitution.md',
						'  456  01-01-2026 00:00   spec-kit/.claude/commands/specify.md',
						'  789  01-01-2026 00:00   spec-kit/.claude/commands/tasks.md',
						'  999  01-01-2026 00:00   spec-kit/.claude/commands/implement.md',
					].join('\n'),
					stderr: '',
				};
			}

			const destMatch = command.match(/-d "([^"]+)"/);
			if (destMatch) {
				const destDir = destMatch[1];
				await fs.mkdir(destDir, { recursive: true });
				if (command.includes('constitution.md')) {
					await fs.writeFile(path.join(destDir, 'constitution.md'), '# Constitution', 'utf8');
				}
				if (command.includes('specify.md')) {
					await fs.writeFile(path.join(destDir, 'specify.md'), '# Specify', 'utf8');
				}
			}
		...
		await expect(fs.stat(promptsPath('speckit.tasks.md'))).rejects.toThrow();
		...
		expect(mocks.logger.warn).toHaveBeenCalledWith('Failed to extract tasks', '[SpecKit]');
	});
	...
	it('surfaces refresh failures and still cleans temporary download state', async () => {
		...
		vi.stubGlobal(
			...
			vi.fn().mockResolvedValueOnce({
				...
				json: vi.fn().mockResolvedValue({
					tag_name: 'v1.0.0',
					assets: [{ name: 'template-vscode.zip', browser_download_url: 'https://example.test' }],
				}),
			})
		);
		await expect(withMockBoundaryTimeout(refreshSpeckitPrompts(), 'missing asset')).rejects.toThrow(
		...
		await expect(
			withMockBoundaryTimeout(refreshSpeckitPrompts(), 'download failure')
