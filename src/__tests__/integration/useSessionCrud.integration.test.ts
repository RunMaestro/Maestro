/**
 * Integration tests for useSessionCrud hook
 *
 * Tests:
 *   - addNewSession (opens new agent choice modal)
...
describe('useSessionCrud', () => {
	...
	describe('createNewSession', () => {
		...
		it('rejects duplicate sessions via validation', async () => {
			...
			expect(logger.error).toHaveBeenCalledWith('Session validation failed: Duplicate session');
		});
		...
		it('uses fallback validation message when validation has no error', async () => {
			...
			expect(logger.error).toHaveBeenCalledWith('Session validation failed: null');
		});
		...
		it('handles agent not found', async () => {
			...
			expect(useSessionStore.getState().sessions).toHaveLength(0);
			expect(logger.error).toHaveBeenCalledWith('Agent not found: unknown-agent');
		});
		...
		it('logs unexpected session creation failures', async () => {
			...
			expect(useSessionStore.getState().sessions).toHaveLength(0);
			expect(logger.error).toHaveBeenCalledWith('Failed to create session:', undefined, error);
		});
	...
	describe('deleteWorktreeGroup', () => {
		...
		it('continues even if playbook deletion fails', async () => {
			...
			expect(useSessionStore.getState().sessions).toHaveLength(0);
			expect(logger.error).toHaveBeenCalledWith('Failed to delete playbooks:', undefined, error);
		});
	});
