...
describe('LeaderboardRegistrationModal integration', () => {
	...
	it('handles expired confirmations and transient polling failures without leaving the interval running', async () => {
		...
		await waitFor(() =>
			expect(consoleWarn).toHaveBeenCalledWith('Polling error:', 'temporary polling error')
		);
		await waitFor(() =>
			expect(consoleWarn).toHaveBeenCalledWith('Poll request failed:', expect.any(Error))
		);
