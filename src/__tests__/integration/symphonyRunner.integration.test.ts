...
describe('symphony runner integration', () => {
	...
	it('starts a direct contribution and prepares local plus external Auto Run documents', async () => {
		...
		expect(result).toEqual({
			...
			draftPrNumber: 42,
			autoRunPath: path.posix.join(localPath, 'Auto Run Docs'),
			isFork: false,
			forkSlug: undefined,
		});
		...
		await expect(
			fs.readFile(path.join(localPath, 'Auto Run Docs', 'task.md'), 'utf-8')
		).resolves.toBe('# Local task\n');
		await expect(
			fs.readFile(path.join(localPath, 'Auto Run Docs', 'external.md'), 'utf-8')
	...
	it('cancels contributions with local cleanup, cross-fork repo routing, and close failure errors', async () => {
		...
		await fs.mkdir(localPath, { recursive: true });
		vi.mocked(execFileNoThrow).mockResolvedValueOnce(ok());
