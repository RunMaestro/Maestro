...
describe('Usage Listener', () => {
	...
	describe('Participant Usage', () => {
		...
		it('should preserve participant context fields when accumulated tokens exceed the context window', async () => {
	...
	describe('Moderator Usage', () => {
		...
		it('should use the fallback context window for moderator usage when the reported window is zero', () => {
			setupListener();
			const handler = eventHandlers.get('usage');
			const usageStats = createMockUsageStats({ contextWindow: 0 });
		...
		it('should preserve moderator context fields when accumulated tokens exceed the context window', () => {
			mockDeps.usageAggregator.calculateContextTokens = vi.fn().mockReturnValue(250000);
			setupListener();
			const handler = eventHandlers.get('usage');
			const usageStats = createMockUsageStats({ contextWindow: 100000 });
