...
vi.mock('../../cli/services/storage', () => ({
	...
	getConfigDirectory: vi.fn(() => '/tmp/maestro-cli-test'),
	readSettingValue: vi.fn(() => undefined),
}));

vi.mock('../../cli/services/prompt-loader', () => ({
	getCliPrompt: vi.fn(async () => 'Test Maestro CLI prompt'),
}));

vi.mock('../../cli/services/git-utils', () => ({
	isGitRepo: vi.fn(() => false),
	getGitBranch: vi.fn(() => undefined),
}));
	...
	it('should query an agent and return JSON response for new session', async () => {
		...
		expect(spawnAgent).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				appendSystemPrompt: 'Test Maestro CLI prompt',
			})
		);
