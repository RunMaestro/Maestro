...
describe('ClaudeSessionStorage direct integration', () => {
	...
	it('covers remote session listing, pagination, read, and search boundaries', async () => {
		...
		testState.readDirRemote.mockResolvedValue({
			success: true,
			data: [
				{ name: 'remote-ok.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'remote-second.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'remote-dir.jsonl', isDirectory: true, isSymlink: false },
				{ name: 'notes.txt', isDirectory: false, isSymlink: false },
				{ name: 'stat-fails.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'stat-throws.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'oversized.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'read-fails.jsonl', isDirectory: false, isSymlink: false },
				{ name: 'read-throws.jsonl', isDirectory: false, isSymlink: false },
			],
		});
		...
		expect(testState.statRemote).toHaveBeenCalledWith(`${remoteDir}/remote-ok.jsonl`, sshConfig);
		...
		testState.readDirRemote.mockResolvedValueOnce({ success: false, error: 'no dir' });
		...
		let remoteOkReads = 0;
		testState.readDirRemote.mockResolvedValueOnce({
			success: true,
			data: [{ name: 'remote-ok.jsonl', isDirectory: false, isSymlink: false }],
		});
		testState.statRemote.mockResolvedValueOnce({
			success: true,
			data: { size: 120, mtime: 3000 },
		});
