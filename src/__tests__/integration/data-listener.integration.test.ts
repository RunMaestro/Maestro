...
describe('Data Listener', () => {
	...
	describe('Event Registration', () => {
		...
		it('should register the raw stdout event listener', () => {
			setupListener();
			expect(mockProcessManager.on).toHaveBeenCalledWith('raw-stdout', expect.any(Function));
		});
	});

	describe('Raw Stdout Streaming', () => {
		it('should emit live participant output only for parsed group chat participant sessions', () => {
			setupListener();
			const handler = eventHandlers.get('raw-stdout');

			handler?.('session-regular', 'ignored output');
			expect(mockOutputParser.parseParticipantSessionId).not.toHaveBeenCalled();

			vi.mocked(mockOutputParser.parseParticipantSessionId).mockReturnValueOnce({
				groupChatId: 'test-chat-123',
				participantName: 'TestAgent',
			});
			handler?.('group-chat-test-chat-123-participant-TestAgent-abc123', 'live output');
			...
			vi.mocked(mockOutputParser.parseParticipantSessionId).mockReturnValueOnce(null);
			handler?.('group-chat-test-chat-123-participant-Unknown-abc123', 'ignored group output');

			expect(groupChatEmitters.emitParticipantLiveOutput).toHaveBeenCalledTimes(1);
		});
	});
