...
describe('fileExplorerStore', () => {
	...
	describe('flat file list', () => {
		it('sets filtered file tree data', () => {
			const tree = [
				{
					name: 'src',
					type: 'folder',
			...
			useFileExplorerStore.getState().setFilteredFileTree(tree);

			expect(useFileExplorerStore.getState().filteredFileTree).toBe(tree);
		});
	...
	describe('non-React access', () => {
		...
		it('getFileExplorerActions returns action functions', () => {
			...
			expect(typeof actions.setFilteredFileTree).toBe('function');
