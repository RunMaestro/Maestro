...
describe('GitDiffViewer', () => {
	...
	describe('Layer stack integration', () => {
		...
		it('refreshes the layer handler after mount', () => {
			const onClose = vi.fn();
			mockParseGitDiff.mockReturnValue([createMockParsedFile()]);
	...
	describe('Stats display', () => {
		it('shows additions in tab for text files', () => {
			...
			expect(screen.getByRole('button', { name: /test\.ts.*1/ })).toBeInTheDocument();
		});
		...
		it('shows deletions in tab for text files with deletions', () => {
			...
			expect(screen.getByRole('button', { name: /file2\.ts.*2/ })).toBeInTheDocument();
		});
