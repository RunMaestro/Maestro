...
describe('marketplace IPC integration', () => {
	...
	it('imports a freshly fetched official playbook with no assets and no target folder prefix', async () => {
		const manifest: MarketplaceManifest = {
			lastUpdated: '2026-05-25',
			playbooks: [
				{
		...
		}>('marketplace:importPlaybook', 'fresh-import', '', autoRunDir, 'session-fresh');

		expect(importResult).toMatchObject({
			success: true,
			importedDocs: ['Phase-01-Fresh'],
			importedAssets: [],
			playbook: { documents: [{ filename: 'Phase-01-Fresh', resetOnCompletion: true }] },
		});
	});
	...
	it('keeps importing when local documents and assets fail', async () => {
		...
		expect(playbooks.playbooks).toHaveLength(2);
		statSpy.mockRestore();
	});
	...
	it('falls back to local import behavior when SSH settings are unavailable', async () => {
		await writeLocalPlaybookManifest();
		fetchMock.mockRejectedValue(new Error('network disabled for local-only import'));
		await registerMarketplace();

		const importResult = await invoke<{ success: true; importedDocs: string[] }>(
			'marketplace:importPlaybook',
			'local-playbook',
		...
		expect(importResult.importedDocs).toEqual(['Phase-01-Setup']);
