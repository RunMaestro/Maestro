...
vi.mock('../../web/mobile/CommandInputBar', () => ({
	CommandInputBar: (props: any) => (
		<div data-testid="command-input-bar">
			...
			<button onClick={() => props.onModeToggle('terminal')}>terminal mode</button>
...
describe('MobileApp integration', () => {
	...
	it('routes connected session actions through App state and desktop bridges', async () => {
		...
		fireEvent.click(screen.getByText('filter auto'));
		...
		expect(mocks.addUserLogEntry).toHaveBeenCalledWith('ship it', 'ai');
		...
		fireEvent.click(screen.getByText('terminal mode'));
		expect(mocks.send).toHaveBeenCalledWith({
			type: 'switch_mode',
			sessionId: 'session-1',
			mode: 'terminal',
	...
	it('keeps command drafts stable for repeated values and empty clears', () => {
		...
		const terminalInput = screen.getByLabelText('mobile command');
	...
	it('renders loading, empty, terminal, and header state variants', () => {
		...
		expect(screen.getByTestId('message-history')).toHaveTextContent('pwd output');
	...
	it('renders dashboard header and session-level metadata fallbacks', () => {
	...
	it('renders missing-tab and high-context header fallbacks', () => {
		mocks.sessionManagement.activeSession = session({
		...
		expect(screen.getByTitle('Session idle')).toBeInTheDocument();
		expect(screen.getByTitle('Context: 95%')).toBeInTheDocument();
		expect(screen.getByLabelText('mobile command')).toHaveValue('');
		...
		expect(screen.getByTitle('Context: 75%')).toBeInTheDocument();
	});
	...
	it('opens history without a selected session', () => {
		mocks.viewState.savedState = {
			...mocks.viewState.savedState,
		...
		expect(screen.getByTestId('history-panel')).toHaveAttribute('data-session-id', 'none');
	});
