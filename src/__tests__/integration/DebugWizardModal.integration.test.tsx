...
const playbooksPath = (directory: string) => `${directory}/${PLAYBOOKS_DIR}`;
...
describe('DebugWizardModal', () => {
	...
	it('logs folder picker failures without changing fields', async () => {
		...
		await waitFor(() => {
			expect(loggerError).toHaveBeenCalledWith('Failed to select directory:', undefined, failure);
		});
		expect(screen.getByPlaceholderText('/path/to/project')).toHaveValue('');
		expect(screen.getByPlaceholderText('My Project')).toHaveValue('');
	});
	...
	it('shows an Auto Run Docs folder error when the directory cannot be read', async () => {
		...
		expect(
			await screen.findByText(`No Auto Run Docs folder found at ${playbooksPath('/repo/missing')}`)
		).toBeInTheDocument();
	...
	it('requires at least one markdown file in the Auto Run Docs folder', async () => {
		...
		expect(
			await screen.findByText(`No markdown files found in ${playbooksPath('/repo/project')}`)
		).toBeInTheDocument();
	...
	it('loads markdown documents, seeds wizard state, and jumps to phase review', async () => {
		...
		expect(maestro.fs.readDir).toHaveBeenCalledWith(playbooksPath('/repo/project'));
		expect(maestro.fs.readFile).toHaveBeenCalledWith(
			`${playbooksPath('/repo/project')}/Phase-01.md`
		);
	...
	it('submits with Enter and reports missing Auto Run Docs folders', async () => {
		...
		expect(
			await screen.findByText(`No Auto Run Docs folder found at ${playbooksPath('/repo/project')}`)
		).toBeInTheDocument();
	});
	...
	it('reports unknown errors for non-Error failures after documents load', async () => {
		...
		expect(loggerError).toHaveBeenCalledWith('Failed to load documents:', undefined, failure);
	});
});
