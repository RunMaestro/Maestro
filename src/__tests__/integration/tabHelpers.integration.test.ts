...
describe('tabHelpers integration', () => {
	it('builds and repairs unified tab order while preserving orphaned AI and file tabs', () => {
	...
	it('handles stale unified file history and duplicate unified AI restores', () => {
		...
		expect(fallbackToFile.closedTabEntry.unifiedIndex).toBe(1);
		expect(fallbackToFile.session.activeFileTabId).toBe('sibling-file');
	...
	it('navigates AI tabs by active, unread, draft, busy, index, and last-tab semantics', () => {
		...
		expect(setActiveTab(alreadyActive, 'idle')!.session).toBe(alreadyActive);
		...
		expect(navigateToLastTab(source, true)!.tab.id).toBe('image-draft');
	...
	it('navigates unified AI and file tabs with repair, wraparound, unread filtering, and invalid refs', () => {
		...
		expect(navigateToUnifiedTabByIndex(source, 1)).toBeNull();
		expect(navigateToUnifiedTabByIndex(source, 0)!.session.unifiedTabOrder).toHaveLength(5);

		const fileResult = navigateToUnifiedTabByIndex(source, 2)!;
		expect(fileResult).toMatchObject({ type: 'file', id: 'file-1' });
		expect(fileResult.session.activeFileTabId).toBe('file-1');

		const aiResult = navigateToUnifiedTabByIndex({ ...source, activeFileTabId: 'file-1' }, 3)!;
		...
		expect(navigateToNextUnifiedTab({ ...source, activeTabId: 'ai-1' }, true)!.id).toBe('file-1');
	...
	it('handles unified navigation guard clauses, stale refs, and filtered fallbacks', () => {
		...
		expect(navigateToUnifiedTabByIndex(invalidAiRef, 0)).toBeNull();
		...
		expect(navigateToUnifiedTabByIndex(activeFile, 1)!.session).toBe(activeFile);
		...
		expect(navigateToPrevUnifiedTab({ ...unreadNavigation, activeTabId: 'ai-2' }, true)!.id).toBe(
			'file-1'
		);
	...
	it('creates merged sessions with real defaults and merged context logs', () => {
		...
		expect(merged.session).toMatchObject({
			...
			activeFileTabId: null,
			autoRunFolderPath: '/workspace/project/Auto Run Docs',
		});
		...
		expect(merged.session.unifiedTabOrder).toEqual([{ type: 'ai', id: merged.tabId }]);
	});
});
