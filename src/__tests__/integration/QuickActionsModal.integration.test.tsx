...
describe('QuickActionsModal integration', () => {
	...
	it('executes git, external-link, debug, ecosystem, and search commands', async () => {
		...
		expect(window.maestro.shell.openExternal).toHaveBeenCalledWith('https://runmaestro.ai/discord');
		...
		await chooseAction(/Enable Auto-Scroll AI Output/i, 'auto-scroll');
		expect(props.setAutoScrollAiMode).toHaveBeenCalledWith(true);
		...
		await waitFor(() => expect(useUIStore.getState().outputSearchOpen).toBe(true));
	...
	it('executes session and group state updater callbacks', async () => {
		...
		await chooseAction(/Clear Terminal History/i, 'clear terminal');
		const clearTerminalUpdater = latestMockArg<(prev: Session[]) => Session[]>(props.setSessions);
		expect(clearTerminalUpdater([activeSession, secondAgent])).toEqual([
			expect.objectContaining({ id: 'child', shellLogs: [] }),
			expect.objectContaining({ id: 'second', shellLogs: secondAgent.shellLogs }),
		]);
	...
	it('covers move-to-root, missing active group, and missing active-session fallbacks', async () => {
		...
		expect(props.setSessions).toHaveBeenCalledWith(
			expect.arrayContaining([expect.objectContaining({ id: 'child', groupId: '' })])
		);
	...
	it('handles repository browser and fallback debug package failures', async () => {
		window.maestro.shell.openExternal = vi.fn().mockRejectedValue(new Error('browser rejected'));
		...
		window.maestro.debug.createPackage = vi
			...
			.mockRejectedValueOnce(new Error('debug rejected'));
		...
		await chooseAction(/Create Debug Package/i, 'debug package');
		await waitFor(() => expect(window.maestro.debug.createPackage).toHaveBeenCalledTimes(3));
	...
	it('handles install GUID missing and rejected clipboard paths', async () => {
		...
		await chooseAction(/Debug: Copy Install GUID to Clipboard/i, 'debug');
		await waitFor(() =>
			expect(consoleWarnSpy).toHaveBeenCalledWith('[Debug] No installation GUID found')
		);
