...
describe('conversationManager integration', () => {
	...
	it('uses Windows stdin flags and exports conversation helper data shapes', async () => {
		...
		expect(spawnPayload.sendPromptViaStdin).toBe(true);
		expect(spawnPayload.sendPromptViaStdinRaw).toBe(false);
		expect(spawnPayload.args[inputFormatIndex + 1]).toBe('stream-json');
		exitListener?.(sessionId, 0);
		...
		expect(stdinPayload.args).toEqual(expect.arrayContaining(['--input-format', 'stream-json']));
