...
function createSession(overrides: Partial<Session> = {}): Session {
	const tab = createTab(overrides.aiTabs?.[0] ? overrides.aiTabs[0] : undefined);
...
describe('TerminalOutput integration', () => {
	...
	it('computes scroll snapshots and terminal highlight markup', () => {
		...
		const darkMarkup = addTerminalHighlightMarkers('Alpha beta alpha', 'alpha', '#f59e0b', 'dark');
		expect(darkMarkup.match(/<mark/g)).toHaveLength(2);
		expect(darkMarkup).toContain('color: #000');

		const lightMarkup = addTerminalHighlightMarkers('Alpha beta', 'alpha', '#f59e0b', 'light');
		expect(lightMarkup).toContain('color: #fff');
		expect(addTerminalHighlightMarkers('Alpha beta', '', '#f59e0b', 'dark')).toBe('Alpha beta');
	});
	...
	it('routes AI log actions through callbacks, clipboard, queue rendering, and save modal state', async () => {
		...
		fireEvent.click(screen.getByTitle(/Show plain text/));
	...
	it('handles terminal mode search UI, local filters, collapse controls, keyboard scrolling, and scroll callbacks', async () => {
		...
		const outputSearchInput = screen.getByPlaceholderText('Search output... (Esc to close)');
		...
		const expandedOutput = screen
			.getByText('Show less')
			.parentElement?.querySelector<HTMLElement>('.whitespace-pre');
		...
		fireEvent.click(screen.getByTitle('Using plain text'));
		expect(screen.getByPlaceholderText('Exclude by RegEx')).toBeInTheDocument();
		const filterInput = screen.getByPlaceholderText('Exclude by RegEx');
		const clearFilterButton = filterInput.parentElement?.querySelector('button:last-child');
		expect(clearFilterButton).toBeTruthy();
		fireEvent.click(clearFilterButton as HTMLButtonElement);
		await waitFor(() =>
	...
	it('covers search escape registration, light search highlights, and terminal delete labels', async () => {
		const setOutputSearchOpen = vi.fn();
		const setOutputSearchQuery = vi.fn();
		const onDeleteLog = vi.fn().mockReturnValueOnce(999).mockReturnValueOnce(null);
		...
		await waitFor(() =>
			expect(screen.getByPlaceholderText('Search output... (Esc to close)')).toBeInTheDocument()
		);
		const lightHighlight = [...document.querySelectorAll('span')].find(
			(element) =>
				element.textContent?.toLowerCase() === 'light' &&
				element.getAttribute('style')?.includes('255, 255, 255')
		);
		...
		fireEvent.click(screen.getByTitle('Delete command and output'));
		fireEvent.click(screen.getByRole('button', { name: 'Yes' }));
		expect(onDeleteLog).toHaveBeenCalledWith('cmd-1');
		await new Promise((resolve) => setTimeout(resolve, 60));
		fireEvent.click(screen.getByTitle('Delete command and output'));
		fireEvent.click(screen.getByRole('button', { name: 'Yes' }));
		expect(onDeleteLog).toHaveBeenCalledTimes(2);
	});
	...
	it('renders terminal busy elapsed time and restores saved scroll position', async () => {
		const session = createSession({
			inputMode: 'terminal',
			state: 'busy',
			busySource: 'terminal',
		...
		expect(screen.getByText(/1:0[5-6]/)).toBeInTheDocument();
		await waitFor(() => expect(screen.getByText(/1:0[6-7]/)).toBeInTheDocument(), {
			timeout: 1500,
		});
		...
		renderTerminal({
			session: {
				...session,
				statusMessage: 'Long command...',
				thinkingStartTime: Date.now() - 3_665_000,
			},
		});
		expect(screen.getByText('Long command...')).toBeInTheDocument();
		expect(screen.getByText(/1:01:0[5-6]/)).toBeInTheDocument();
	});
	...
	it('expands terminal user output without parent scroll capture at boundaries', async () => {
		...
		const expandedOutput = screen
			.getByText('Show less')
			.parentElement?.querySelector<HTMLElement>('.whitespace-pre-wrap');
		expect(expandedOutput).toBeTruthy();
		setScrollGeometry(expandedOutput as HTMLElement, {
			scrollTop: 0,
			scrollHeight: 500,
	...
	it('renders default busy and queued fallbacks plus remote save modal state', async () => {
		renderTerminal({
			session: createSession({
				inputMode: 'terminal',
				state: 'busy',
