	...
	return {
		...
		parseJsonObject: vi.fn((parsed: any) => {
			if (parsed?.type === 'result') {
				return { type: 'result', text: parsed.text ?? parsed.result };
			}
			if (parsed?.type === 'text') {
				return { type: 'text', text: parsed.text, isPartial: !!parsed.isPartial };
			}
			return null;
		}),
...
describe('ChildProcessSpawner integration', () => {
	...
	it('spawns an interactive process and routes stdio through the real handlers', () => {
		...
		expect(events).toContainEqual(['raw-stdout', 'session-1', 'plain output']);
		expect(bufferManager.emitDataBuffered).toHaveBeenCalledWith('session-1', 'plain output');
		expect(events).toContainEqual(['stderr', 'session-1', 'warning output']);
		expect(events).toContainEqual(['exit', 'session-1', 0]);
		expect(processes.has('session-1')).toBe(false);
	});

	it('accumulates batch output, emits metadata, and records environment context', () => {
		const { events, spawner } = createContext();
		...
		expect(mocks.buildChildProcessEnv).toHaveBeenCalledWith({ LOCAL_ONLY: '1' }, false, {
		...
		expect(events).toContainEqual(['data', 'session-1', 'batch result']);
	...
	it('uses file image arguments and prompt-embedded resume prompts for non-stream-json agents', () => {
		...
		expect(mocks.buildChildProcessEnv).toHaveBeenCalledWith(undefined, true, undefined);
