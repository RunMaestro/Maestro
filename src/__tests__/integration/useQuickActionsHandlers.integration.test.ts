 ...
 *   - handleQuickActionsRefreshGitFileState: calls refreshGitFileState, refreshWorktreeState, mainPanelRef.refreshGitInfo, sets and clears flash notification
...
describe('useQuickActionsHandlers', () => {
	...
	describe('handleQuickActionsRefreshGitFileState', () => {
		...
		it('emits a center flash with the expected message', async () => {
			const session = createSession({ id: 'sess-1' });
			useSessionStore.setState({ sessions: [session], activeSessionId: 'sess-1' });
			...
			expect(notifyCenterFlash).toHaveBeenCalledWith({
				message: 'Files, Git, History Refreshed',
				color: 'theme',
			});
		});

		it('does not emit a second flash when the cleanup timer runs', async () => {
			vi.useFakeTimers();
		...
		it('does not emit a center flash when there is no active session', async () => {
			useSessionStore.setState({ sessions: [], activeSessionId: '' });
