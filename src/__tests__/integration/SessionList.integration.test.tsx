...
describe('SessionList', () => {
	...
	describe('Worktree Sessions', () => {
		it('shows a collapsed worktree band and expands it through the parent callback', () => {
			...
			expect(screen.getByText('Feature Alpha')).not.toBeVisible();

			fireEvent.click(screen.getByRole('button', { name: 'Expand worktrees' }));
		...
		it('renders expanded worktree children and collapses them through the drawer handle', () => {
			...
			expect(screen.getByRole('button', { name: 'Collapse worktrees' })).toHaveAttribute(
				'aria-expanded',
				'true'
			);
	...
	describe('Context Menu', () => {
		...
		it('routes edit and duplicate context actions to their handlers', () => {
			...
			expect(useModalStore.getState().getData('newInstance')?.duplicatingSessionId).toBe('s1');
	...
	describe('Session Status Indicators', () => {
		...
		it('shows busy status with pulse animation', () => {
			...
			// Busy status renders an animated pulse ring behind the status dot.
	...
	describe('Achievement Badge', () => {
		...
		it('marks the collapsed wand as active while a session is busy', () => {
			const sessions = [
				createMockSession({ id: 's1', state: 'busy', agentSessionId: 'claude-session-1' }),
			];
			useSessionStore.setState({ sessions });
			useUIStore.setState({ leftSidebarOpen: false });
	...
	describe('Focus Management', () => {
		...
		it('shows focus ring when activeFocus is sidebar', () => {
			...
			expect(container.firstChild).toHaveStyle({
				boxShadow: 'inset -1px 0 0 #bd93f9, inset 1px 0 0 #bd93f9, inset 0 -1px 0 #bd93f9',
			});
		});
	});
	...
	describe('Resize Handle', () => {
		...
		it('clamps sidebar width within bounds', () => {
			...
			// Try to drag below the sidebar drag minimum (280px)
			fireEvent.mouseDown(resizeHandle!, { clientX: 300 });
