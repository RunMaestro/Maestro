...
import { getAutoRunWatcherCount, registerAutorunHandlers } from '../../main/ipc/handlers/autorun';
...
describe('Auto Run IPC integration', () => {
	...
	it('rejects unsafe local inputs and handles filesystem edge cases', async () => {
		...
		await expectFailure('autorun:readDoc', [docsDir, 'missing'], 'File not found');
		await expect(
		...
		await expectFailure('autorun:deleteFolder', [projectWithFile], 'not a directory');
		...
		appHandlers.get('before-quit')?.();
		expect(getAutoRunWatcherCount()).toBe(0);
