...
describe('useMainKeyboardHandler integration', () => {
	...
	it('routes App-level shortcuts through the current ref context', () => {
		...
		dispatchKey({ shortcutId: 'toggleAutoScroll' });
		...
		expect(context.addNewSession).toHaveBeenCalled();
		...
		expect(context.toggleInputMode).toHaveBeenCalled();
		...
		expect(context.setSettingsTab).toHaveBeenCalledWith('general');
		...
		expect(context.setPromptComposerOpen).toHaveBeenCalledWith(true);
		...
		expect(context.setAutoScrollAiMode).toHaveBeenCalledWith(true);
	...
	it('integrates modal and settings stores for tab and font-size shortcuts', () => {
		...
		expect(context.navigateToUnifiedTabByIndex).toHaveBeenCalledWith(initialActiveSession, 1);
		expect(context.navigateToLastUnifiedTab).toHaveBeenCalledWith(initialActiveSession);
	});
	...
	it('covers tab close variants and no-op tab management fallbacks', () => {
		...
		const fallbackContext = createContext({
			...
			getActiveTab: vi.fn(() => ({ id: 'no-agent', title: 'No Agent', logs: [] })),
		});
	...
	it('covers layer-gated allowances and keyboard navigation early returns', () => {
		...
		expect(modalAllowanceContext.setAutoScrollAiMode).toHaveBeenCalledWith(true);
		expect(useSettingsStore.getState().fontSize).toBe(16);
		...
		expect(overlayAllowanceContext.toggleInputMode).toHaveBeenCalled();
	...
	it('covers branch-only shortcut fallbacks and alternate modifier combinations', () => {
		...
		dispatchKey({ tabShortcutId: 'goToTab1' });
		dispatchKey({ tabShortcutId: 'goToLastTab' });
		expect(unreadOnlyContext.setSessions).not.toHaveBeenCalled();
