...
vi.mock('../../main/agents/opencode-config', () => ({
...
describe('agent-detector', () => {
	...
	describe('detectAgents', () => {
		...
		it('should detect all defined agent types', async () => {
			...
			expect(agentIds).toEqual([...AGENT_IDS].sort());
		});
	...
	describe('discoverModels', () => {
		...
		it('should return empty array for agents that do not support model selection', async () => {
			// Setup: terminal is available but does not support model selection
			mockExecFileNoThrow.mockImplementation(async (cmd, args) => {
				const binaryName = args[0];
				if (binaryName === 'claude') {
					return { stdout: '/usr/bin/claude\n', stderr: '', exitCode: 0 };
			...
			const models = await detector.discoverModels('terminal');
		...
		it('should return empty array when no model discovery command is implemented', async () => {
			...
			const models = await detector.discoverModels('factory-droid');

			expect(models).toEqual([]);
			expect(logger.debug).toHaveBeenCalledWith(
				expect.stringContaining('No model discovery implemented for factory-droid'),
				'AgentDetector'
			);
		});
		...
		it('should handle model discovery command failure', async () => {
			...
			expect(models).toEqual([]);
			expect(logger.warn).toHaveBeenCalledWith(
				expect.stringContaining('CLI model discovery failed for opencode'),
				'AgentDetector',
				expect.objectContaining({ stderr: 'command failed' })
			);
		});
	...
	describe('pathless detection fallbacks', () => {
		it('should log Windows diagnostics and discover models with the agent command when no path is resolved', async () => {
			...
			try {
				const { AgentDetector: IsolatedAgentDetector } = await import('../../main/agents/detector');
