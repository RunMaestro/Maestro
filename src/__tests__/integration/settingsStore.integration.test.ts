...
const defaultStatePatch: SettingsStoreState = {
...
describe('settingsStore integration', () => {
	...
	it('persists a representative settings session through the Electron settings bridge', () => {
		...
		actions.setEnterToSendTerminal(false);
		...
		actions.setAutoScrollAiMode(true);
	...
	it('validates constrained settings before writing them back to persisted settings', () => {
		...
		actions.setConductorProfile('a'.repeat(1200));
		...
		expect(getSettingsState().conductorProfile).toHaveLength(1000);
		...
		expect(getSettingsState().documentGraphLayoutType).toBe('mindmap');
		expect(getSettingsState().moderatorStandingInstructions).toHaveLength(2000);
		expect(window.maestro.settings.set).toHaveBeenCalledWith('conductorProfile', 'a'.repeat(1000));
	...
	it('marks settings loaded when the batch-load IPC fails', async () => {
		...
		expect(errorSpy).toHaveBeenCalledWith('[Settings] Failed to load settings:', expect.any(Error));
	});
});
