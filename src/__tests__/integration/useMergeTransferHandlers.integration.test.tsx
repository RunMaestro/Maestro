...
describe('useMergeTransferHandlers integration', () => {
	...
	beforeEach(() => {
		...
		vi.mocked(gitService.getStatus).mockResolvedValue({ branch: 'feature/integration' } as any);
	...
	it('creates a target tab, sends filtered context to the target agent, and handles git fallback', async () => {
		const source = session();
		...
		expect(vi.mocked(gitService.getStatus)).toHaveBeenCalledWith(target.cwd);
	...
	it('captures git status failures while still spawning the target agent', async () => {
		const source = session();
		const target = targetSession({ isGitRepo: true });
		const { result } = renderHandlers([source, target], source.id);
		...
		await act(async () => {
			await result.current.handleSendToAgent(target.id, {
				targetSessionId: target.id,
				groomContext: false,
			});
			await flushAsyncTurns();
		...
		expect(captureException).toHaveBeenCalledWith(
			expect.any(Error),
			expect.objectContaining({
				extra: expect.objectContaining({ operation: 'git-status-for-transfer' }),
			})
		);
		expect(window.maestro.process.spawn).toHaveBeenCalledOnce();
	});
