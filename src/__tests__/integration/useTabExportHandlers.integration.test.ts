...
// Mock center-flash copy feedback
...
describe('useTabExportHandlers', () => {
	...
	describe('handleCopyContext', () => {
		it('writes formatted logs to clipboard on success', async () => {
			...
			expect(mockFormatLogsForClipboard).toHaveBeenCalledWith(tab.logs, { includeThinking: false });
		...
		it('logs the error when clipboard write fails', async () => {
	...
	describe('handleExportHtml', () => {
		...
		it('logs the error when export throws', async () => {
			...
			expect(mockLoggerError).toHaveBeenCalledWith('Failed to export tab:', undefined, exportError);
		});
