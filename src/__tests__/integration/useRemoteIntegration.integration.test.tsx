...
describe('useRemoteIntegration integration', () => {
	...
	it('logs generic agent-session persistence failures for non-Claude tab metadata', async () => {
		...
		expect(console.error).toHaveBeenCalledWith('Failed to persist tab name:', expect.any(Error));
		expect(console.error).toHaveBeenCalledWith('Failed to persist tab starred:', expect.any(Error));
	});
