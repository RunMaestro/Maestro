...
describe('useSshRemotes integration', () => {
	...
	it('normalizes thrown load errors from Error and non-Error values', async () => {
		...
		expect(consoleError).toHaveBeenCalledWith('[useSshRemotes] Failed to load configs:', loadError);
		first.unmount();
		...
		const second = await mountHook();
		expect(second.result.current.error).toBe('Failed to load SSH remote configurations');
		expect(consoleError).toHaveBeenCalledWith('[useSshRemotes] Failed to load configs:', 'offline');
	});
	...
	it('normalizes save failure and exception paths', async () => {
		...
		expect(consoleError).toHaveBeenCalledWith('[useSshRemotes] Failed to save config:', saveError);
		...
		expect(consoleError).toHaveBeenCalledWith('[useSshRemotes] Failed to save config:', 'offline');
	});
