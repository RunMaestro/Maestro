...
describe('ProcessManager integration', () => {
	...
	it('kills all active processes and returns false when lifecycle methods throw', () => {
		const manager = new ProcessManager();
		...
		const failingPty = createPty();
		failingPty.kill.mockImplementationOnce(() => {
			throw new Error('kill failed');
		});
		setManagedProcess(
			failingManager,
			createManagedProcess({ isTerminal: true, ptyProcess: failingPty, sessionId: 'failing' })
		);
