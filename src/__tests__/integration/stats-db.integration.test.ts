...
let throwOnOpen:
	| ((dbPath: string, options: Record<string, unknown> | undefined) => Error | null)
	| null = null;
...
function createStatement(sql: string) {
	return {
		...
		get: vi.fn(() => {
			...
			if (sql.includes('MIN(startTime)') && sql.includes('query_events')) {
				return { earliest: earliestQuery };
			}
			if (sql.includes('MIN(startTime)') && sql.includes('auto_run_sessions')) {
				return { earliest: earliestAutoRun };
			}
			if (sql.includes('MIN(createdAt)') && sql.includes('session_lifecycle')) {
...
describe('StatsDB integration', () => {
	...
	it('initializes, migrates, vacuums, backs up, restores, and reports timestamps', async () => {
		...
		expect(statsDb.getCurrentVersion()).toBe(4);
		expect(statsDb.getTargetVersion()).toBe(4);
