...
describe('Logger', () => {
	...
	function mockFileLogging(
		...
		fsMock.createWriteStream.mockImplementation(() => {
			...
			return { write: fsMock.write, end: fsMock.end, on: fsMock.on };
		});
	...
	describe('File Logging', () => {
		it('should use APPDATA and enable file logging by default on Windows', async () => {
			...
			try {
				...
				expect(windowsLogger.getLogFilePath()).toMatch(/maestro-debug-\d{4}-\d{2}-\d{2}\.log$/);
		...
		it('should enable file logging, create the log directory, and disable the stream', async () => {
			...
			expect(logger.getLogFilePath()).toMatch(/maestro-debug-\d{4}-\d{2}-\d{2}\.log$/);
			expect(mkdirSync).toHaveBeenCalledWith(expect.any(String), { recursive: true });
			expect(createWriteStream).toHaveBeenCalledWith(logger.getLogFilePath(), {
				fd: 123,
				autoClose: true,
			});
