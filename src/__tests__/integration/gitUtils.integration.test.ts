...
describe('gitUtils', () => {
	...
	describe('parseGitNumstat', () => {
		...
		it('ignores malformed numstat rows', () => {
			const output = '10\t5\n3\t1\tsrc/valid.ts\n';
			expect(parseGitNumstat(output)).toEqual([
				{ path: 'src/valid.ts', additions: 3, deletions: 1 },
			]);
		});
	...
	describe('sanitizeGitBranchName', () => {
		it('trims whitespace and replaces illegal branch characters', () => {
			expect(sanitizeGitBranchName('  main  ')).toBe('main');
			expect(sanitizeGitBranchName('Feature Branch')).toBe('Feature-Branch');
			expect(sanitizeGitBranchName('feature/foo\n')).toBe('feature/foo');
			expect(sanitizeGitBranchName('bad:name')).toBe('bad-name');
		});

		it('handles empty/null input', () => {
			expect(sanitizeGitBranchName('')).toBe('');
			expect(sanitizeGitBranchName(null as unknown as string)).toBe('');
			expect(sanitizeGitBranchName(undefined as unknown as string)).toBe('');
		});
	});
