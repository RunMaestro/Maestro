...
describe('UsageDashboardModal integration', () => {
	...
	it('handles export cancellation, export failure, and control hover states', async () => {
		...
		fireEvent.click(exportButton);
		await waitFor(() => {
			expect(consoleError).toHaveBeenCalledWith('Failed to export CSV:', exportError);
		});
	...
	it('switches dashboard views and supports section keyboard navigation', async () => {
		...
		expect(await screen.findByTestId('section-summary-cards')).toBeInTheDocument();
	...
	it('exercises tab and section keyboard handlers across every dashboard view', async () => {
		...
		expect(await screen.findByTestId('section-activity-heatmap')).toBeInTheDocument();
		...
		fireEvent.keyDown(screen.getByTestId('section-summary-cards'), {
			key: 'Tab',
			shiftKey: true,
		});
		...
		fireEvent.keyDown(tablist, { key: 'Tab' });
		fireEvent.keyDown(screen.getByTestId('section-agent-comparison'), { key: 'ArrowUp' });
		expect(document.activeElement).toBe(screen.getByTestId('section-summary-cards'));
		...
		fireEvent.keyDown(screen.getByTestId('section-duration-trends'), { key: 'End' });
		fireEvent.keyDown(screen.getByTestId('section-duration-trends'), { key: 'ArrowDown' });

		fireEvent.click(tabs[1]);
		expect(await screen.findByTestId('section-session-stats')).toBeInTheDocument();
		const agentSections = ['session-stats', 'agent-efficiency', 'agent-comparison', 'agent-usage'];
		...
		fireEvent.click(tabs[3]);
		expect(await screen.findByTestId('section-autorun-stats')).toBeInTheDocument();
		const autoRunSections = ['autorun-stats', 'tasks-by-hour', 'longest-autoruns'];
	...
	it('handles real-time stats updates, empty data, and recoverable fetch errors', async () => {
		...
		expect(await screen.findByText('Failed to load usage data')).toBeInTheDocument();
		expect(consoleError).toHaveBeenCalledWith('Failed to fetch usage stats:', 'stats unavailable');
