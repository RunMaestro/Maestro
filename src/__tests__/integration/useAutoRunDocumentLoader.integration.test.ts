...
describe('useAutoRunDocumentLoader', () => {
	...
	describe('onFileChanged event handling', () => {
		...
		it('reloads selected file content when changed file matches autoRunSelectedFile', async () => {
			...
			// Call sequence:
			// 1. initial task-count read, reused as selected file content
			// 2. file-change task-count read, reused as selected file content
			mockReadDoc
			...
			await waitFor(() => {
				...
				expect(updated?.autoRunContent).toBe('initial content');
			});
		...
		it('increments autoRunContentVersion on file change content reload', async () => {
			...
			mockReadDoc
				.mockResolvedValueOnce({ success: true, content: 'initial content' })
				.mockResolvedValueOnce({ success: true, content: 'new content from disk' });
