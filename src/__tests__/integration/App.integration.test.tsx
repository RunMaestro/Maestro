...
describe('MaestroConsole integration', () => {
	...
	it('renders the empty app shell and routes menu actions into app-level modals', async () => {
		...
		expect(mockLazyState.settingsProps?.initialTab).toBe('general');
	...
	it('mounts active group chat panels and switches to the log viewer through modal props', async () => {
		...
		try {
			...
			expect(useUIStore.getState().successFlashNotification).toBe('Group chat copied');
			act(() => {
	...
	it('mounts file-scoped lazy workflows for marketplace, gist publishing, and document graph', async () => {
		...
		expect(window.maestro.shell.openExternal).toHaveBeenCalledWith('https://example.com/spec');
		await act(async () => {
		...
		await waitFor(() =>
			expect(mockMainPanelState.latestProps?.slashCommands).toEqual(
				expect.arrayContaining([
					...
					expect.objectContaining({ command: '/openspec-proposal', isOpenspec: true }),
				])
			)
		);
		expect(
	...
	it('renders fallback chrome, drag, and capability states', async () => {
		...
		act(() => {
			fireEvent.dragEnter(container.firstElementChild as Element, {
	...
	it('reports group-chat Auto Run trigger failures back to the moderator', async () => {
		...
		await waitFor(() => expect(maestro.groupChat.onAutoRunTriggered).toHaveBeenCalled());
		const triggerAutoRun = maestro.groupChat.onAutoRunTriggered.mock.calls[0][0];

		maestro.groupChat.reportAutoRunComplete.mockRejectedValueOnce(new Error('moderator down'));
		await act(async () => {
			triggerAutoRun('chat-1', 'Missing Agent');
			await Promise.resolve();
		});
		expect(maestro.groupChat.reportAutoRunComplete).toHaveBeenLastCalledWith(
			'chat-1',
			'Missing Agent',
			expect.stringContaining('No Maestro agent named "Missing Agent" found')
		);
		expect(errorSpy).toHaveBeenCalledWith(
		...
		useSessionStore.setState({
			...
			sessions: [createSession({ name: 'Planner', autoRunFolderPath: undefined })],
		});
		act(() => {
		...
		expect(maestro.groupChat.reportAutoRunComplete).toHaveBeenLastCalledWith(
			'chat-1',
			'Planner',
			expect.stringContaining('has no Auto Run folder configured')
		);

		useSessionStore.setState({
			activeSessionId: 'session-1',
			sessions: [createSession({ name: 'Planner', autoRunFolderPath: '/docs' })],
		});
		maestro.autorun.listDocs.mockResolvedValueOnce({ files: [] });
		await act(async () => {
			triggerAutoRun('chat-1', 'Planner');
			await Promise.resolve();
		});
		expect(maestro.groupChat.reportAutoRunComplete).toHaveBeenLastCalledWith(
			'chat-1',
			'Planner',
			expect.stringContaining('No Auto Run documents found')
		);
		...
		maestro.autorun.listDocs.mockRejectedValueOnce(new Error('disk unavailable'));
		await act(async () => {
			triggerAutoRun('chat-1', 'Planner');
			await Promise.resolve();
		});
		expect(maestro.groupChat.reportAutoRunComplete).toHaveBeenLastCalledWith(
			'chat-1',
			'Planner',
			expect.stringContaining('Failed to read Auto Run folder')
		);

		maestro.autorun.listDocs.mockResolvedValueOnce({ files: ['alpha', 'beta'] });
		maestro.groupChat.reportAutoRunComplete.mockClear();
		await act(async () => {
		...
		maestro.groupChat.reportAutoRunComplete.mockClear();
		await act(async () => {
			triggerAutoRun('chat-1', 'Planner', 'beta.md');
			await Promise.resolve();
		});
		expect(maestro.groupChat.reportAutoRunComplete).not.toHaveBeenCalled();
