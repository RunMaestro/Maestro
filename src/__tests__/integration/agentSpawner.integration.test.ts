...
describe('agent spawner integration', () => {
	...
	it('rejects JSON-line capable agents when no parser exists', async () => {
		...
		await expect(spawnAgent('gemini-cli' as never, '/repo', 'prompt')).rejects.toThrow(
			'No parser available for agent type: gemini-cli'
		);
	});
});
