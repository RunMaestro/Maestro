...
describe('MainPanel integration', () => {
	...
	it('drives file preview callbacks and exposed focus/refresh handles', async () => {
		...
		expect(screen.getByText('File Preview: readme.md')).toBeInTheDocument();
		...
		await waitFor(() =>
			expect(window.maestro.fs.writeFile).toHaveBeenCalledWith('/repo/docs/readme.md', 'saved')
		);
	...
	it('drives git tooltip copy, browser, and worktree actions', async () => {
		...
		expect(mockSafeClipboardWrite).toHaveBeenCalledWith('claude-session-1234');
	...
	it('renders settings, capability, and context fallback states', async () => {
		...
		expect(screen.getByTitle('Click to copy: claude-session-1234')).toBeInTheDocument();
		...
		fireEvent.click(screen.getByTitle('Click to copy: claude-session-1234'));
		await waitFor(() => expect(mockSafeClipboardWrite).toHaveBeenCalledWith('claude-session-1234'));
	...
	it('renders loading, wizard, and stopping auto-run fallback states', async () => {
		render(
			<MainPanel
				{...props({
					filePreviewLoading: { name: 'remote.md', path: '/repo/remote.md' },
				})}
			/>
		);
		expect(screen.getByText('Loading remote.md')).toBeInTheDocument();
