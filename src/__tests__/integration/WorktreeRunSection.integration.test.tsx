...
describe('WorktreeRunSection', () => {
	...
	beforeEach(() => {
		vi.clearAllMocks();
		vi.mocked(gitService.getBranches).mockResolvedValue(['main', 'develop']);
	...
	it('shows error message when getBranches fails', async () => {
		...
		const scanMock = vi.fn().mockResolvedValue({ gitSubdirs: [] });
		(window.maestro.git as Record<string, unknown>).scanWorktreeDirectory = scanMock;
		vi.mocked(gitService.getBranches).mockRejectedValueOnce(new Error('git not found'));
	...
	it('ignores branch-loading failures after unmount', async () => {
		...
		vi.mocked(gitService.getBranches).mockImplementation(branchesMock);
		...
		await waitFor(() => {
			expect(branchesMock).toHaveBeenCalledWith('/project');
		});
		unmount();
