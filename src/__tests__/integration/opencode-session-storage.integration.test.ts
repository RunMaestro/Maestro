...
describe('OpenCodeSessionStorage', () => {
	...
	it('prefers XDG-style OpenCode storage paths on Windows when no candidate exists', async () => {
		...
		try {
			...
			expect(windowsStorage.getSessionPath('C:\\repo\\project', 'ses_win')).toBe(
				path.join(mocks.homeDir, '.local', 'share', 'opencode', 'storage', 'message', 'ses_win')
			);
		} finally {
	...
	it('uses the Windows APPDATA fallback when that OpenCode directory exists', async () => {
		const originalAppData = process.env.APPDATA;
		try {
			process.env.APPDATA = 'C:\\Users\\test\\AppData\\Roaming';
			const appDataOpenCodeDir = path.join(process.env.APPDATA, 'opencode');
			mocks.existsSync.mockImplementation((candidate: string) => candidate === appDataOpenCodeDir);
			vi.mocked(isWindows).mockReturnValue(true);
			vi.resetModules();
			...
			expect(windowsStorage.getSessionPath('C:\\repo\\project', 'ses_win')).toBe(
				path.join(appDataOpenCodeDir, 'storage', 'message', 'ses_win')
			);
		} finally {
			if (originalAppData === undefined) {
				delete process.env.APPDATA;
