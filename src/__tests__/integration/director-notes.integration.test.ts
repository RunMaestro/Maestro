...
import { sanitizeDisplayName } from '../../main/utils/director-notes-prompt';
...
vi.mock('../../main/prompt-manager', () => ({
	getPrompt: vi.fn(() => 'Mock director notes prompt'),
}));
...
describe('director-notes IPC handlers', () => {
	...
	beforeEach(() => {
		...
		vi.mocked(mockHistoryManager.getEntries).mockReturnValue([
			createMockEntry({ id: 'recent-entry', timestamp: Date.now() }),
		]);
