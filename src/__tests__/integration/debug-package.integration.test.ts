...
import type { DebugPackageDependencies } from '../../main/debug-package';
...
import { generateDebugPackage, previewDebugPackage } from '../../main/debug-package';
...
} as unknown as DebugPackageDependencies;
...
function resetCollectorDefaults() {
	...
	vi.mocked(deps.groupsStore.get).mockReturnValue([{ id: 'group-1' }]);
}
...
describe('debug package integration', () => {
	...
	it('keeps generating when collectors fail and includes collection errors', async () => {
		...
		vi.mocked(deps.groupsStore.get).mockImplementation(() => {
	...
	it('previews the diagnostic categories included in debug packages', () => {
