...
describe('useInputProcessing integration', () => {
	...
	it('handles guards and expected command handler failures without dispatching agent work', async () => {
		...
		await act(async () => rendered.result.current.processInput());
		await act(async () => flushAsyncTurns());
		expect(consoleError).toHaveBeenCalledWith('[processInput] Wizard message failed:', wizardError);
	});
	...
	it('queues write messages while Auto Run is active', async () => {
		...
		expect(consoleLog).toHaveBeenCalledWith(
			...
			expect.objectContaining({ isAutoRunActive: true, shouldQueue: true })
		);
	});
	...
	it('bypasses the write queue only when busy and queued work is read-only', async () => {
		...
		expect(consoleLog).toHaveBeenCalledWith(
			'[processInput] Queue decision:',
			expect.objectContaining({ shouldQueue: false, queueLength: 1 })
		);
		...
		expect(consoleLog).toHaveBeenCalledWith(
			'[processInput] Queue decision:',
			expect.objectContaining({ shouldQueue: true, queueLength: 1 })
		);
		...
		expect(consoleLog).toHaveBeenCalledWith(
			'[processInput] Queue decision:',
			expect.objectContaining({ shouldQueue: true })
		);
	});
	...
	it('spawns batch agents with read-only, nudge, image, and session override context', async () => {
		...
		expect(consoleLog).toHaveBeenCalledWith(
			'[processInput] Queue decision:',
			expect.objectContaining({ isReadOnlyMode: true, shouldQueue: false })
		);
	});

	it('builds new batch prompts with image-only, merged-context, system-prompt, and history context', async () => {
		const consoleLog = vi.spyOn(console, 'log').mockImplementation(() => {});
		const consoleWarn = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const getStatusSpy = vi
			.spyOn(gitService, 'getStatus')
			.mockResolvedValue({ branch: 'feature/input-processing' } as any);
		const tab = createTab({
			agentSessionId: null,
	...
	it('updates automatic tab names from quick matches, generated names, null responses, and failures', async () => {
		...
		expect(consoleLog).toHaveBeenCalledWith(
			'[processInput] Queue decision:',
			expect.objectContaining({ shouldQueue: false })
		);
	});
	...
	it('records process failures for batch spawns, terminal commands, and stdin writes', async () => {
		...
		expect(consoleError).toHaveBeenCalledWith('Failed to run command:', expect.any(Error));
		expect(updated[1].state).toBe('idle');
		...
		expect(consoleError).toHaveBeenCalledWith('Failed to write to process:', expect.any(Error));
		expect(updated[1].state).toBe('idle');
