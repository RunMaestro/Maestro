...
describe('AutoRunDocumentSelector integration', () => {
	...
	it('opens the flat document menu, selects a document, and runs adjacent controls', () => {
		...
		expect(screen.getByRole('button', { name: /alpha\.md/i })).toHaveTextContent('50%');
		...
		fireEvent.click(screen.getByTitle('Enable Bionify for this document preview'));
	...
	it('renders empty, loading, and enabled Bionify states without invoking disabled actions', () => {
		const onRefresh = vi.fn();
		const onToggleBionify = vi.fn();
		renderSelector({
			bionifyEnabled: true,
		...
		fireEvent.click(screen.getByRole('button', { name: /alpha\.md/i }));
		expect(screen.getByRole('button', { name: /alpha\.md/i })).toHaveTextContent('100%');
		expect(screen.getByText('No markdown files found')).toBeInTheDocument();

		const bionify = screen.getByTitle('Disable Bionify for this document preview');
		expect(bionify).toHaveAttribute('aria-pressed', 'true');
		fireEvent.click(bionify);
		expect(onToggleBionify).toHaveBeenCalledTimes(1);
...
function renderSelector(overrides: Partial<ComponentProps<typeof AutoRunDocumentSelector>> = {}) {
	return render(<AutoRunDocumentSelector {...createProps(overrides)} />);
}
