/**
 * Integration tests for web mobile TabBar component
 *
 * @module TabBar.test
 */
...
describe('TabBar', () => {
	...
	describe('Render conditions', () => {
		it('renders pinned controls when tabs array is empty', () => {
			render(
				<TabBar
					tabs={[]}
					activeTabId="tab-1"
		...
		it('renders pinned controls and the single tab when there is only one tab', () => {
			render(
				<TabBar
					tabs={[defaultTab]}
					activeTabId="tab-1"
	...
	describe('New tab button', () => {
		...
		it('opens the new-tab menu and calls onNewTab from New AI Chat', () => {
			const tabs = [
				createTab({ id: 'tab-1', name: 'First' }),
				createTab({ id: 'tab-2', name: 'Second' }),
			];
			...
			fireEvent.click(screen.getByRole('button', { name: /new ai chat/i }));
