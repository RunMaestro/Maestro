...
describe('MessageHistory', () => {
	...
	describe('Message Truncation', () => {
		...
		const manyLines = Array.from({ length: 15 }, (_, index) => `Line ${index + 1}`).join('\n');
		const maxOutputLines = 8;

		it('truncates text longer than maxOutputLines', () => {
			const logs: LogEntry[] = [createLogEntry({ text: longText })];
			render(<MessageHistory logs={logs} inputMode="ai" maxOutputLines={maxOutputLines} />);
			expect(screen.getByText('▶ expand')).toBeInTheDocument();
		});

		it('truncates text with more lines than maxOutputLines', () => {
		...
		it('expands truncated message on click', () => {
			...
			const { container } = render(
				<MessageHistory logs={logs} inputMode="ai" maxOutputLines={maxOutputLines} />
			);
		...
		it('does not truncate by character count when under the line cap', () => {
