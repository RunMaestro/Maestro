...
describe('remote-fs integration', () => {
	...
	it('writes, creates, renames, deletes, scans, and retries transient SSH failures', async () => {
		...
		const writeCommand = execSsh.mock.calls
			.map(([, args]) => commandFrom(args as string[]))
			.find((command) => command.includes('/repo/output.txt'));
		expect(writeCommand).toContain(Buffer.from('Hello remote', 'utf-8').toString('base64'));
	});
