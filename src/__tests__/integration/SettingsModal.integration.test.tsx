/**
 * Integration tests for SettingsModal.tsx
 *
...
vi.mock('../../renderer/components/Settings/tabs/MaestroPromptsTab', () => ({
	MaestroPromptsTab: ({ initialSelectedPromptId }: { initialSelectedPromptId?: string }) => (
		<div data-testid="maestro-prompts-tab">
			Maestro Prompts Tab
...
const mockSetGlobalShowHotkey = vi.fn();
...
vi.mock('../../renderer/hooks/settings/useSettings', () => ({
	useSettings: () => ({
		...
		// Shortcut settings
		globalShowHotkey: ['Meta', 'Shift', 'M'],
		setGlobalShowHotkey: mockSetGlobalShowHotkey,
		shortcuts: {
		...
		leftPanelCollapsedPillsPerRow: 2,
...
const getAudioFeedbackTestButton = () => screen.getAllByRole('button', { name: 'Test' })[0];
...
describe('SettingsModal', () => {
	...
	beforeEach(() => {
		...
		consoleLogSpy = vi.spyOn(console, 'log').mockImplementation((...args: unknown[]) => {
			if (String(args[0] ?? '').startsWith('[Notification]')) return;
	...
	describe('tab navigation', () => {
		...
		it('should switch to Maestro Prompts tab', async () => {
			render(<SettingsModal {...createDefaultProps()} />);
			...
			fireEvent.click(screen.getByTitle('Maestro Prompts'));

			await act(async () => {
				await vi.advanceTimersByTimeAsync(50);
			});

			expect(screen.getByTestId('maestro-prompts-tab')).toBeInTheDocument();
		});
	...
	describe('Display tab - File panel settings', () => {
		it('should call setFileExplorerMaxDepth when the depth slider changes', async () => {
			const setFileExplorerMaxDepth = vi.fn();
			mockUseSettingsOverrides = { setFileExplorerMaxDepth };
			render(<SettingsModal {...createDefaultProps({ initialTab: 'display' })} />);
			...
			fireEvent.change(screen.getByLabelText('Max recursion depth'), { target: { value: '12' } });
			expect(setFileExplorerMaxDepth).toHaveBeenCalledWith(12);
		});

		it('should call setFileExplorerMaxEntries from preset buttons', async () => {
			const setFileExplorerMaxEntries = vi.fn();
			mockUseSettingsOverrides = { setFileExplorerMaxEntries };
			render(<SettingsModal {...createDefaultProps({ initialTab: 'display' })} />);
			...
			expect(setFileExplorerMaxEntries).toHaveBeenCalledWith(10000);
		});
	});
	...
	describe('General tab - Input behavior toggles', () => {
		...
		it('should call setEnterToSendAIExpanded when toggled', async () => {
			render(<SettingsModal {...createDefaultProps()} />);
			...
			const terminalModeLabel = screen.getByText('Expanded AI Interaction Mode');
			const terminalModeSection = terminalModeLabel.closest('.p-3');
			const toggleButton = terminalModeSection?.querySelector('button');
			fireEvent.click(toggleButton!);

			expect(mockSetEnterToSendAIExpanded).toHaveBeenCalledWith(false);
		});
	...
	describe('Notifications tab', () => {
		...
		it('should test Command Chain when test button is clicked', async () => {
			...
			fireEvent.click(getAudioFeedbackTestButton());
	...
	describe('Display tab - SSH file panel cap controls', () => {
		it('should call setSshReduceEntryCapEnabled when the SSH cap toggle changes', async () => {
			const setSshReduceEntryCapEnabled = vi.fn();
			mockUseSettingsOverrides = { setSshReduceEntryCapEnabled };
			render(<SettingsModal {...createDefaultProps({ initialTab: 'display' })} />);
			...
			fireEvent.click(screen.getByRole('checkbox', { name: /Reduce entry cap on SSH remotes/i }));
			expect(setSshReduceEntryCapEnabled).toHaveBeenCalledWith(false);
		});

		it('should call setSshReduceEntryCapFraction when the SSH cap slider changes', async () => {
			const setSshReduceEntryCapFraction = vi.fn();
			mockUseSettingsOverrides = { setSshReduceEntryCapFraction };
			render(<SettingsModal {...createDefaultProps({ initialTab: 'display' })} />);
			...
			fireEvent.change(screen.getByLabelText('SSH cap fraction'), { target: { value: '0.75' } });
			expect(setSshReduceEntryCapFraction).toHaveBeenCalledWith(0.75);
		});
	});
	...
	describe('WakaTime CLI status', () => {
		...
		it('should check CLI when WakaTime is enabled and the CLI is unavailable', async () => {
			vi.useRealTimers();
			mockUseSettingsOverrides = { wakatimeEnabled: true };
			...
			await expect(
				vi.mocked(window.maestro.wakatime.checkCli).mock.results[0].value
