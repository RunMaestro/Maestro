...
describe('documentStats integration', () => {
	...
	it('computes stats while falling back from unexpected parser and string-operation failures', async () => {
		const parserWarn = vi.spyOn(console, 'warn').mockImplementation(() => {});
		vi.resetModules();
		...
		expect(parserWarn).toHaveBeenCalledWith(
		...
		const fallback = computeDocumentStats(
