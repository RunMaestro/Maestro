/**
 * Integration tests for useSessionLifecycle hook (Phase 2H)
 *
...
describe('useSessionLifecycle', () => {
		...
		it('updates session with all agent configuration fields', () => {
			...
			act(() => {
				result.current.handleSaveEditAgent(
					...
					'nudge msg',
					undefined, // newSessionMessage unchanged
					'/custom/path',
					'--arg1',
					{ MY_VAR: 'value' },
					'gpt-4',
	...
	describe('navigation history edge cases', () => {
		it('tracks the active AI tab when no higher-priority tab id exists', () => {
			const tab = createMockAITab({ id: 'tab-1' });
			const session = createMockSession({
				id: 'session-1',
				inputMode: 'terminal' as any,
