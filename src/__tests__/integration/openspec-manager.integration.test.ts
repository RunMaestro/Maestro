...
describe('openspec manager integration', () => {
	...
	it('refreshes upstream prompts from mocked release and AGENTS.md responses', async () => {
		await writeJson(customizationsPath(), {
		...
		const fetchMock = stubFetch(
			...
			{
				ok: true,
				text: vi.fn().mockResolvedValue(fullAgentsMd()),
			}
		);
		...
		expect(fetchMock).toHaveBeenCalledWith(
			'https://raw.githubusercontent.com/Fission-AI/OpenSpec/v9.8.7/openspec/AGENTS.md'
		);
	...
	it('surfaces refresh failures and warns when AGENTS.md sections are missing', async () => {
		stubFetch(new Error('network down'), {
			ok: false,
			statusText: 'Not Found',
		});
		await expect(refreshOpenSpecPrompts()).rejects.toThrow('Failed to fetch AGENTS.md: Not Found');
		expect(mocks.logger.warn).toHaveBeenCalledWith(
