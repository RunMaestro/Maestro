...
describe('ExecutionQueueIndicator', () => {
	...
	const getQueueButton = (): HTMLButtonElement => {
		const button = screen.getByText(/items? queued/).closest('button');
		expect(button).not.toBeNull();
		return button as HTMLButtonElement;
	};

	const getIndicatorContainer = (): HTMLElement => {
		const container = getQueueButton().parentElement;
	...
	describe('tab grouping and pills', () => {
		...
		it('should not show count for tabs with single item', () => {
			...
			render(<ExecutionQueueIndicator session={session} theme={theme} onClick={mockOnClick} />);
			const pill = screen.getByText('MyTab');
		...
		it('should group items by tab and show multiple pills', () => {
			...
			expect(container).toHaveTextContent('items queued');
			// The first tab (TabA with 2 items) should always be visible
			expect(container).toHaveTextContent('TabA (2)');
			...
			const hasTabB = container.textContent?.includes('TabB');
			const hasOverflowIndicator = container.textContent?.includes('+1');
		...
		it('handles a sparse queue without tab names during resize calculation', async () => {
			...
			expect(getQueueButton()).toHaveTextContent('1');
			expect(getQueueButton()).toHaveTextContent('item queued');
			await waitFor(() => {
	...
	describe('styling', () => {
		it('should apply theme colors to container', () => {
			...
			render(<ExecutionQueueIndicator session={session} theme={theme} onClick={mockOnClick} />);
			const button = getIndicatorContainer();
	...
	describe('accessibility', () => {
		it('should be a button element for keyboard accessibility', () => {
			...
			render(<ExecutionQueueIndicator session={session} theme={theme} onClick={mockOnClick} />);
			expect(getQueueButton()).toBeInTheDocument();
		});
