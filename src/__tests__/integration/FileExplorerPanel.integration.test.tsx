...
describe('FileExplorerPanel integration coverage', () => {
	...
	it('covers header controls, hidden-file filtering, and error fallback actions', async () => {
		...
		expect(props.onShowFlash).toHaveBeenCalledWith('Path copied to clipboard');
	...
	it('covers filter layer handlers and file row interaction branches', () => {
		...
		const activeFileSession = createSession({
			...
			changedFiles: [
				{ path: '/Users/test/project/package.json', type: 'modified' },
				{ path: '/Users/test/project/README.md', type: 'deleted' },
			],
			filePreviewTabs: [
				{
					content: '{}',
	...
	it('warns and skips duplicate sibling and duplicate full paths', () => {
		...
		expect(warnSpy).toHaveBeenCalledWith('[FileExplorer] Duplicate sibling skipped:', '', 'dup.ts');
		expect(warnSpy).toHaveBeenCalledWith('[FileExplorer] Duplicate path skipped:', 'src/utils.ts');
		warnSpy.mockRestore();
	});
