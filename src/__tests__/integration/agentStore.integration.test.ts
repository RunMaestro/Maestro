/**
 * Integration tests for agentStore - Agent lifecycle orchestration store
 *
...
import { loadInputProcessingPrompts } from '../../renderer/hooks/input/useInputProcessing';
...
const mockGetPrompt = vi.fn(async (promptId: string) => ({
	success: true,
	content:
		promptId === 'image-only-default' ? 'Describe this image' : 'Mock system prompt for {{CWD}}',
}));
const mockGetHistoryFilePath = vi.fn().mockResolvedValue('/test/.maestro/history/session-1.jsonl');
...
(window as any).maestro = {
	...
	history: {
		getFilePath: mockGetHistoryFilePath,
	},
};
...
describe('agentStore', () => {
	...
	describe('clearAgentError', () => {
		...
		it('handles IPC clearError rejection without throwing', async () => {
			...
			expect(loggerSpy).toHaveBeenCalledWith('Failed to clear agent error:', undefined, error);
	...
	describe('non-React access', () => {
		...
		it('getAgentActions returns all action functions', () => {
			...
			expect(typeof actions.loadCapabilitySnapshots).toBe('function');
			expect(typeof actions.getCapabilitySnapshot).toBe('function');
			expect(typeof actions.reprobeAgent).toBe('function');
			...
			// Verify exactly 13 actions (no extras, no missing)
	...
	describe('processQueuedItem', () => {
		...
		it('prepends system prompt for new sessions (no agentSessionId)', async () => {
			...
			// System prompt is passed through the dedicated appendSystemPrompt spawn field.
