...
describe('useTabHandlers integration', () => {
	...
	it('routes mixed tab close commands through confirmation state and unified ordering', () => {
		...
		expect(activeSession().aiTabs[0]).toMatchObject({ agentSessionId: null, logs: [] });
	...
	it('covers AI tab close confirmations, current-tab returns, reorders, and non-Claude star persistence', async () => {
		...
		expect(useModalStore.getState().getData('confirm')?.message).toContain('Close this wizard');
		act(() => useModalStore.getState().getData('confirm')!.onConfirm());
	...
	it('covers inactive-session updates, close callbacks, persistence failures, and successful navigation branches', async () => {
		...
		await Promise.resolve();
		expect(errorSpy).toHaveBeenCalledWith('Failed to persist tab starred:', expect.any(Error));
	...
	it('covers remaining no-op guards and close-range confirmation branches', async () => {
		...
		act(() => hook.result.current.handleCloseTabsLeft());
		expect(useModalStore.getState().getData('confirm')?.message).toContain('unsent drafts');
		act(() =>
			setDualSession(
				session({
					id: 'active-session',
		...
		try {
			...
			expect(activeSession().aiTabs[0].starred).toBe(false);
		} finally {
			act(() => {
