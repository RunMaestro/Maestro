...
describe('CommandInputBar integration', () => {
	...
	it('submits AI drafts while handling recent commands, swipe history, and keyboard viewport changes', async () => {
		...
		expect(onSubmit).toHaveBeenCalledWith('summarize the logs');
	...
	it('guards disabled connection states, empty terminal submits, and desktop AI keyboard submit', () => {
		...
		expect(onSubmit).toHaveBeenCalledWith('desktop prompt');
	...
	it('expands the phone AI composer on focus, submits with the full-width action, and collapses on outside touch', async () => {
		...
		expect(onSubmit).toHaveBeenCalledWith('draft message');
		await waitFor(() =>
	...
	it('opens the real long-press quick actions menu and routes mode switching', async () => {
		vi.useFakeTimers({ shouldAdvanceTime: true });
		const onModeToggle = vi.fn();
		renderBar({
			value: 'queued prompt',
		...
		fireEvent.click(screen.getByRole('menuitem', { name: /Switch to Terminal/i }));
		expect(onModeToggle).toHaveBeenCalledWith('terminal');
		expect(vibrateSpy).toHaveBeenCalledWith(25);
