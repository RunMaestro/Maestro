...
describe('useAnnouncement hook', () => {
	...
	it('debounces rapid announcements and announces only the latest message', () => {
		vi.useFakeTimers();

		function DebouncedComponent() {
			const { announce, announcementProps } = useAnnouncement(50);

			return (
				<div>
					<button onClick={() => announce('First announcement')}>First</button>
					<button onClick={() => announce('Second announcement', 'assertive')}>Second</button>
					<ScreenReaderAnnouncement {...announcementProps} />
				</div>
			);
		}
		...
		try {
			...
			expect(screen.getByText('Second announcement')).toHaveAttribute('aria-live', 'assertive');
		} finally {
			vi.useRealTimers();
		}
	});
