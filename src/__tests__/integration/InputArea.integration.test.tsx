...
describe('InputArea integration', () => {
	...
	it('drives AI-mode completions, image staging, context warning, and footer actions', async () => {
		...
		fireEvent.click(
			(screen.getByAltText('Staged image 1').closest('button') as HTMLButtonElement)
		...
		fireEvent.doubleClick(screen.getByText('/help'));
	...
	it('covers command history escape, tab-completion empty states, remote cwd, and terminal toggles', () => {
		...
		fireEvent.click(screen.getByTitle(/Toggle Mode/));
		fireEvent.click(screen.getByTitle('Run command (Enter)'));
		expect(toggleInputMode).toHaveBeenCalledTimes(1);
		expect(processInput).toHaveBeenCalledTimes(1);
	...
	it('handles optional completion callbacks, empty uploads, missing reader results, and wizard mode icon', async () => {
