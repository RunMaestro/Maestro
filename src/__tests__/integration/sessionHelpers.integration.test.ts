...
describe('sessionHelpers', () => {
	beforeEach(() => {
		vi.clearAllMocks();
		mockPromptsApi.get.mockResolvedValue({ success: true, content: '' });

		// Setup window.maestro mock
		(global as any).window = {
			maestro: {
				agents: mockAgentsApi,
				prompts: mockPromptsApi,
			},
		};
	});
	...
	describe('buildSpawnConfigForAgent', () => {
		it('returns null when agent is not found', async () => {
			...
			expect(loggerSpy).toHaveBeenCalledWith('[sessionHelpers] Agent not found: unknown-agent');
		...
		it('returns null when agent is not available', async () => {
			...
			expect(result).toBeNull();
			expect(loggerSpy).toHaveBeenCalledWith('[sessionHelpers] Agent not available: claude-code');
			loggerSpy.mockRestore();
		});
	...
	describe('createSessionForAgent', () => {
		...
		it('returns null when spawn config cannot be built', async () => {
			...
			expect(loggerSpy).toHaveBeenCalledWith('[sessionHelpers] Agent not found: claude-code');
			loggerSpy.mockRestore();
		});
