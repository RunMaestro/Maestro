...
describe('ExistingAutoRunDocsModal integration', () => {
	...
	it('confirms the focused action from keyboard navigation', () => {
		...
		const dialog = screen.getByRole('dialog', { name: 'Existing Auto Run Documents Detected' });
		...
		const tabDialog = screen.getByRole('dialog', { name: 'Existing Auto Run Documents Detected' });
		...
		const arrowDialog = screen.getByRole('dialog', {
			name: 'Existing Auto Run Documents Detected',
		});
