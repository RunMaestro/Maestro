...
describe('fileExplorer integration', () => {
	...
	it('loads local trees with gitignore patterns, duplicate guards, progress, sorting, and child failures', async () => {
		...
		const tree = await loadFileTree('/repo', 10, 0, undefined, progress, {
			honorGitignore: true,
			ignorePatterns: [],
		});
		...
		expect(errorSpy).toHaveBeenCalledWith('Error loading file tree:', expect.any(Error));
	...
	it('loads remote trees with remote gitignore success, empty, and failure paths', async () => {
		...
		).resolves.toEqual([{ name: 'file.txt', type: 'file' }]);
		...
		).resolves.toEqual([{ name: 'vendor', type: 'folder', children: [] }]);
		...
		).resolves.toEqual([{ name: 'remote.txt', type: 'file' }]);
		...
		).resolves.toEqual([{ name: 'local.txt', type: 'file' }]);
	});

	it('handles max-depth and root read failures', async () => {
		await expect(loadFileTree('/repo', 1, 1)).resolves.toEqual([]);
