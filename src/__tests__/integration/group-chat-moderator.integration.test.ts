...
describe('group-chat-moderator', () => {
	...
	describe('spawnModerator - system prompt', () => {
		...
		it('synthesis prompt contains synthesis instructions', () => {
			const synthesisPrompt = getModeratorSynthesisPrompt();
			expect(synthesisPrompt).toContain('reviewing responses');
			expect(synthesisPrompt).toContain('AI');
		});
	});
	...
	describe('getModeratorSessionId', () => {
		...
		it('reports whether a moderator session is active', async () => {
			const chat = await createTestChat('Active Session Test', 'claude-code');
