...
async function createBundledOmpFixture(): Promise<{ readonly runtimeSource: string }> {
	...
	if (!runtime) throw new Error('OMP bundle omitted a sandbox entry');
	const runtimeSource = new TextDecoder().decode(runtime.content);
	expect(runtimeSource).not.toMatch(/["']node:/);
	expect(runtimeSource).not.toMatch(/\brequire\s*\(/);
	expect(runtimeSource).not.toMatch(/\bBuffer(?:\.|\()/);
	expect(runtimeSource).not.toMatch(/\bprocess(?:\.|\[)/);
	expect(runtimeSource).not.toMatch(/\bmodule\.require\b/);
