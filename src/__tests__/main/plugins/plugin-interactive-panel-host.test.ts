...
import { OMP_PANEL_BRIDGE_DESCRIPTOR } from '../../../../plugins/com.maestro.omp/src/bridge/descriptor.ts';
...
describe('PluginInteractivePanelHost', () => {
	...
	it('delivers each OMP closed result through the host and rejects extra result fields', async () => {
		const scheduler = createScheduler();
		const host = new PluginInteractivePanelHost({ scheduler });
		const guest = createMount();
		const mounted = host.mount({
			...
			panelLocalId: 'omp',
			generation: 1n,
			descriptor: OMP_PANEL_BRIDGE_DESCRIPTOR as unknown as InteractivePanelDescriptor,
			sender: guest.sender,
		});
		const owner = host.ownerApi('com.maestro.omp', 1n);
		const received: Array<{ requestId: string; kind: string }> = [];
		owner.onRequest((request) =>
			received.push({ requestId: request.requestId, kind: request.kind })
		);
		const snapshot = {
			connection: 'ready',
			models: [],
		...
		const payloads: Record<string, Record<string, unknown>> = {
			'omp.session.create': {},
			'omp.session.select': { sessionId: 'session-1' },
			'omp.prompt.send': { sessionId: 'session-1', text: 'hello', attachments: [] },
			'omp.steer.send': { sessionId: 'session-1', text: 'hello', attachments: [] },
			'omp.followUp.send': { sessionId: 'session-1', text: 'hello', attachments: [] },
			'omp.run.abort': { sessionId: 'session-1' },
			'omp.run.abortAndPrompt': { sessionId: 'session-1', text: 'hello', attachments: [] },
			...
			'omp.approval.resolve': { sessionId: 'session-1', requestId: 'approval-1', approved: true },
			'omp.thinking.set': { sessionId: 'session-1', level: 'medium' },
			'omp.thinking.cycle': { sessionId: 'session-1' },
			'omp.settings.set': { sessionId: 'session-1', setting: 'steeringMode', value: 'all' },
			'omp.commands.refresh': {},
			'omp.messages.load': { sessionId: 'session-1' },
			'omp.stats.load': { sessionId: 'session-1' },
			'omp.subagents.load': { sessionId: 'session-1', subagentId: 'subagent-1' },
			'omp.auth.providers': {},
			'omp.auth.login': { providerId: 'openai' },
			'omp.export.request': { sessionId: 'session-1' },
		};
		const results: Record<string, Record<string, unknown>> = {
			'omp.session.create': snapshot,
			'omp.session.select': snapshot,
			'omp.prompt.send': {},
			'omp.steer.send': {},
			'omp.followUp.send': {},
			'omp.run.abort': {},
			...
			'omp.thinking.set': snapshot,
			'omp.thinking.cycle': snapshot,
			'omp.settings.set': snapshot,
			'omp.commands.refresh': snapshot,
			...
			'omp.auth.providers': {
				providers: [{ id: 'openai', name: 'OpenAI', available: true, authenticated: false }],
			},
			'omp.auth.login': { providerId: 'openai' },
			'omp.export.request': { path: 'session.html' },
		};

		for (const [index, kind] of Object.keys(payloads).entries()) {
			host.receive(guest.sender, 'maestro:panel-request', {
				instanceId: mounted.instanceId,
				requestId: index * 2 + 1,
				kind,
			...
			const validRequest = received.at(-1);
			expect(validRequest?.kind).toBe(kind);
			await owner.resolve(validRequest!.requestId, kind, results[kind] as never);
			expect(guest.send).toHaveBeenLastCalledWith('maestro:panel-result', {
				instanceId: mounted.instanceId,
				requestId: index * 2 + 1,
				kind,
			...
			const invalidRequest = received.at(-1);
			await owner.resolve(invalidRequest!.requestId, kind, {
				...results[kind],
				unexpected: true,
			} as never);
			expect(guest.send).toHaveBeenLastCalledWith('maestro:panel-error', {
