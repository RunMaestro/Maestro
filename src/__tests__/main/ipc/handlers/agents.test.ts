...
describe('agents IPC handlers', () => {
	...
	describe('agents:setClaudeInteractiveMode', () => {
		// Helper: re-register with a fresh sessionsStore mock backed by an in-memory
		// session list. Returns the handler + a `readSessions()` accessor so tests
		// can verify what landed on disk.
		const mountWithSessions = (initial: any[]) => {
			const data: { sessions: any[] } = { sessions: [...initial] };
			const mockSessionsStore = {
				get: vi.fn((key: string, def: any) => {
					if (key === 'sessions') return data.sessions;
					return def;
				}),
				set: vi.fn((key: string, value: any) => {
					if (key === 'sessions') data.sessions = value;
				}),
			};
			handlers.clear();
			registerAgentsHandlers({
			...
			return {
				handler: handlers.get('agents:setClaudeInteractiveMode')!,
				readSessions: () => data.sessions,
				mockSessionsStore,
			};
		};

		it('writes the new claudeInteractive block through to disk', async () => {
			const { handler, readSessions } = mountWithSessions([
				{
					id: 'sess-1',
					name: 'A',
			...
			const result = await handler({} as any, 'sess-1', 'interactive', 'user');

			expect(result).toBe(true);
			const updated = readSessions().find((s) => s.id === 'sess-1');
			expect(updated.claudeInteractive).toEqual({ mode: 'interactive', modeReason: 'user' });
		});

		it('preserves lastUsageSnapshotKey across writes', async () => {
			const { handler, readSessions } = mountWithSessions([
				{
					id: 'sess-1',
					name: 'A',
			...
			await handler({} as any, 'sess-1', 'interactive', 'user');

			const updated = readSessions().find((s) => s.id === 'sess-1');
			expect(updated.claudeInteractive.lastUsageSnapshotKey).toBe('/Users/me/.claude');
		});

		it('no-ops when the new state matches the current state', async () => {
			const { handler, mockSessionsStore } = mountWithSessions([
				{
					id: 'sess-1',
					name: 'A',
					toolType: 'claude-code',
		...
		it('returns false when the target session is missing', async () => {
			const { handler, mockSessionsStore } = mountWithSessions([
				{
					id: 'sess-1',
					name: 'A',
			...
			const result = await handler({} as any, 'does-not-exist', 'interactive', 'user');

			expect(result).toBe(false);
			expect(mockSessionsStore.set).not.toHaveBeenCalled();
		});

		it('returns false when sessionsStore is unwired', async () => {
			handlers.clear();
			registerAgentsHandlers(deps); // no sessionsStore in deps
		...
		it('throws for an invalid mode value', async () => {
			const { handler } = mountWithSessions([
				{ id: 'sess-1', name: 'A', toolType: 'claude-code', cwd: '/foo', projectRoot: '/foo' },
			]);

			await expect(handler({} as any, 'sess-1', 'bogus', 'user')).rejects.toThrow(/Invalid mode/i);
		});

		it('throws for an invalid modeReason value', async () => {
			const { handler } = mountWithSessions([
				{ id: 'sess-1', name: 'A', toolType: 'claude-code', cwd: '/foo', projectRoot: '/foo' },
			]);

			await expect(handler({} as any, 'sess-1', 'interactive', 'bogus')).rejects.toThrow(
				/Invalid modeReason/i
			);
		});

		it('returns false when the disk write throws', async () => {
			const data = {
				sessions: [
					{ id: 'sess-1', name: 'A', toolType: 'claude-code', cwd: '/foo', projectRoot: '/foo' },
				],
			...
			const mockSessionsStore = {
				get: vi.fn((key: string) => (key === 'sessions' ? data.sessions : undefined)),
				set: vi.fn(() => {
		...
		it('accepts the full enum for modeReason (auto, limit, user)', async () => {
			const { handler } = mountWithSessions([
				{ id: 'sess-1', name: 'A', toolType: 'claude-code', cwd: '/foo', projectRoot: '/foo' },
			]);
