...
describe('process IPC handlers', () => {
	...
	describe('process:spawn', () => {
		...
		it('assigns an unowned spawned session to the sender window before process output can emit', async () => {
			const mockAgent = {
				id: 'claude-code',
				requiresPty: true,
			};
			...
			expect(mockWebContents.send.mock.invocationCallOrder[0]).toBeLessThan(
