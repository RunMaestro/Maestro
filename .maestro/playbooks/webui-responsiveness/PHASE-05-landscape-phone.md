...
- [ ] **Task 5.2 — Wire `CommandInputBar.compact` to `isShortViewport`.** In the component's parent in `App.tsx`, set `compact={isShortViewport}`. Remove the `TODO(phase-5)` marker placed in Phase 1.
