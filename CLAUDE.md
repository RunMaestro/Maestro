...
| Modify Auto Run Thought Stream      | `src/renderer/stores/thoughtStreamStore.ts` (in-memory capture + `groupThoughtsIntoBlocks` + `buildActivityFeed`), `src/renderer/components/ThoughtStreamPanel.tsx` (panel), `src/renderer/hooks/agent/internal/useThoughtStreamCaptureListener.ts` (taps `process:thinking-chunk`), `src/renderer/hooks/agent/internal/useThoughtStreamToolListener.ts` (taps `process:tool-execution`), `src/renderer/utils/toolActivityLabel.ts` (one-line plain-language tool labels) |
