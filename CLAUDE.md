...
- **Session mutation:** `updateSessionWith(sessionId, updater)` for an agent, `updateAiTab(sessionId, tabId, updater)` for one AI tab, both in `src/renderer/stores/sessionStore.ts` (do NOT hand-roll `setSessions(prev => prev.map(...))`)
...
- **Lane and row positions in the Git Log graph:** `computeGitGraphLanes()`, `stepGitGraphAlongLane()`, `stepGitGraphAcrossLanes()`, `jumpGitGraphAlongLane()`, `gitGraphLaneEdge()`, `pickBranchFromRefs()` in `src/renderer/utils/gitGraphLanes.ts`. `GitGraphView` DRAWS the graph and `GitLogViewer` NAVIGATES it with the arrow keys, and both need the same answer to "which lane is this commit in, and what order are the lanes drawn in?" - derive it twice and an arrow key lands in a column the user is not looking at. Row 0 is the OLDEST commit and is drawn at the BOTTOM (matching @gitgraph's vertical orientation), so a step of `+1` is visually upward. **Each axis answers one question:** Up/Down walk `commitsOnLane` - the commits of the anchor's OWN lane, oldest first - and Left/Right are the only keys that cross to another branch. Do NOT move vertically by the global row order: that drifts sideways across branches on its own, which both leaves Left/Right with nothing to do and makes Home/End/PageUp/PageDown land on a branch the user never asked for (they are lane ends too, via `gitGraphLaneEdge` / `jumpGitGraphAlongLane`, and a jump CLAMPS to the lane's end where a single step holds). Note the graph is built from `git log --all` while the List view holds only the current branch, so graph navigation must move by LANE rather than by the list's index - a commit selected on a side branch has no list index, and the key reads as dead.
