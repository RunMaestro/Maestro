...
- **Which agent a voice session is bound to:** `voiceScopeAgentId(scope)` in `src/shared/acappella/document-scope.ts`, alongside `isDocumentScope()`, `documentScopeName()` and `applyDocumentScope()`. `VoiceScope` has THREE kinds and two of them carry an agent (`agent` and `document`), so the hand-written `scope.kind === 'agent' ? scope.sessionId : null` that used to be copied into six files reports a document conversation as an unbound Conductor session: the HUD relabels itself "Conductor", the Left Bar's talking indicator goes dark on the agent that is actually being talked to, and the router loses its bias. A document scope is an agent scope that also knows what the conversation is ABOUT, and `applyDocumentScope` is the one place its three rules live (it stays with its agent, the opening turn hands the document over, every later turn recalls the tab that conversation opened in). Apply it AFTER routing, never before - the Brain still has to read what was said. **Starting one goes through `talkWithDocument()` in `src/renderer/services/documentVoice.ts`**, the same arrangement as `requestFileDeletion`: the File Preview toolbar microphone, the Files panel menu entry, and the palette's `File: Talk with Document` all route there so the Encore gate, the agent resolution, the HUD un-hiding, and the refusal toast cannot drift. Which files it is OFFERED on is `isTalkableDocumentPath()` in `src/shared/fileKinds.ts` (images including SVG, playable media, and compiled binaries have no text to read), asked once so a file cannot show the microphone in one surface and hide it in another.
...
1. **Nothing starts itself.** Enabling the feature opens no device, downloads nothing, and registers no hotkey. A session begins only from an explicit trigger (composer microphone, global hotkey, command palette, Left Bar menu, wake word, paired phone). The one surface that appears on its own is **Voice Setup** (`Settings/ACappella/VoiceSetupModal.tsx`), offered when the Encore flag is switched on: it is a consent prompt, not a session, it fetches nothing until the user presses Download, and **Later** leaves voice enabled and unconfigured. That is deliberate - the default provider trio is now the LOCAL one (`DEFAULT_PROVIDER_IDS` in `providers/provider-registry.ts`), so without models a session refuses by name instead of transcribing nothing while looking healthy.
...
| Change local speech-to-text          | `src/main/acappella/providers/local/whisper/` (`mel.ts` features, `tokenizer.ts` BPE decode, `engine.ts` encoder + greedy KV-cached loop). Plain TypeScript over ONNX Runtime - there is no whisper.cpp and nothing compiles                                         |
| Add a downloadable native runtime    | `src/shared/acappella/native-runtimes.ts` (descriptor) + `runtime-artifacts.ts` (pinned tarball, hash, and `dependencies`). A tarball is NOT an install: anything the payload `require()`s must be listed, or it verifies clean and throws `MODULE_NOT_FOUND`        |
