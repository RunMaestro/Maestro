...
- **Which account an agent presents:** `resolveCredentialIdentity()` in `src/shared/providerAuth.ts` (pure, callable from both processes). Login state is keyed by CREDENTIAL, not by session: fifteen agents on one Anthropic account share one `CredentialIdentity.key` and therefore one probe, one snapshot, one badge. Read the snapshot with `getSnapshot()` (`src/main/stores/providerAuthStore.ts`) or the renderer hooks in `src/renderer/stores/providerAuthStore.ts` - never spawn your own `claude auth status`. Companions in the same module: `mergeEffectiveEnv(agentLevel, sessionLevel)` for the agent-under-session env precedence (do NOT hand-roll `{ ...agentEnv, ...sessionEnv }` a fourth time), `sshRemoteIdFromHost()` / `isRemoteHost()` instead of slicing `'ssh:'` off a host string, `canonicalizeDirPath()` for config-dir keys, `fingerprintSecret()` for the only representation of a secret allowed out of the module, and `resolveLoginCommand()` which returns `null` for every credential a sign-in cannot repair (that `null` is what removes the button - do not re-derive it from `kind`). See [AGENT-INFRA.md → Provider Auth](docs/agent-guides/AGENT-INFRA.md).
