...
- [x] Support MailTo
  - Added built-in `mailto` scheme parsing and serialization with `to`, `subject`, `body`, and header support.
- [x] Be 100% iso compatible with uri-js
  - Removed the remaining uri-js compatibility skip by normalizing IPv4-like hosts with leading zero octets to match uri-js parse output.
