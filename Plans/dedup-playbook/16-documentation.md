...
| Provider/model lists                 | IDs/capabilities: `src/shared/agentRegistry.ts`; display names: `src/shared/agentMetadata.ts`; public capability guide: `docs/provider-notes.md`.                      | `docs/getting-started.md` and `docs/installation.md` link to the catalog instead of repeating provider lists. Provider Notes retains per-provider capability tables because that is its named public purpose.   | Compare identifiers and names against both registries; render provider, installation, and getting-started pages. Revert each link replacement independently if a public audience loses context. |
